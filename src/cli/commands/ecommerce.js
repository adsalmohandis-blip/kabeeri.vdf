...
function ecommerce(action, value, flags = {}, rest = [], deps = {}) {
  ...
  if (!plugin.enabled) {
    throw new Error(`Ecommerce plugin blocked: ${plugin.status === "disabled" ? "enable" : "install"} ecommerce-builder first with \`kvdf plugins ${plugin.status === "disabled" ? "enable" : "install"} ecommerce-builder\`.`);
  }

  if (action === "init") {
    const description = [value, ...rest, flags.description, flags.goal, flags.text].filter(Boolean).join(" ").trim();
    const project = createEcommerceProject(description, flags, { writeJsonFile, plugin });
    const report = buildEcommerceStatusReport(readEcommerceState(), plugin, project);
  ...
  if (action === "questionnaire") {
    ...
    const report = buildEcommerceStatusReport(nextState, plugin, updatedProject);
    if (flags.json) console.log(JSON.stringify(report, null, 2));
    else renderEcommerceStatusReport(report, table);
    return;
  }
  ...
  if (action === "brief") {
    ...
    if (flags.json) console.log(JSON.stringify(buildEcommerceStatusReport(nextState, plugin, updatedProject), null, 2));
    else renderEcommerceStatusReport(buildEcommerceStatusReport(nextState, plugin, updatedProject), table);
    return;
  }
...
function getEcommercePluginStatus() {
  const report = buildPluginLoaderReport();
  const plugin = report.plugins.find((item) => item.plugin_id === "ecommerce-builder" || item.bundle_path === "plugins/ecommerce-builder" || item.name === "Ecommerce Builder");
  if (!plugin) {
