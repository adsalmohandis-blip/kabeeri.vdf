...
function booking(action, value, flags = {}, rest = [], deps = {}) {
  ...
  if (!plugin.enabled) {
    throw new Error(`Booking plugin blocked: ${plugin.status === "disabled" ? "enable" : "install"} booking-builder first with \`kvdf plugins ${plugin.status === "disabled" ? "enable" : "install"} booking-builder\`.`);
  }

  if (action === "init") {
    const description = [value, ...rest, flags.description, flags.goal, flags.text].filter(Boolean).join(" ").trim();
    const project = createBookingProject(description, flags, { writeJsonFile, plugin });
    const report = buildBookingStatusReport(readBookingState(), plugin, project);
  ...
  if (action === "questionnaire") {
    ...
    const report = buildBookingStatusReport(nextState, plugin, updatedProject);
    if (flags.json) console.log(JSON.stringify(report, null, 2));
    else renderBookingStatusReport(report, table);
    return;
  }
  ...
  if (action === "brief") {
    ...
    if (flags.json) console.log(JSON.stringify(buildBookingStatusReport(nextState, plugin, updatedProject), null, 2));
    else renderBookingStatusReport(buildBookingStatusReport(nextState, plugin, updatedProject), table);
    return;
  }
...
function getBookingPluginStatus() {
  const report = buildPluginLoaderReport();
  const plugin = report.plugins.find((item) => item.plugin_id === "booking-builder" || item.bundle_path === "plugins/booking-builder" || item.name === "Booking Builder");
  if (!plugin) {
