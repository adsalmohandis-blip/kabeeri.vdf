...
const { taskAssessment, readTaskAssessmentsState } = require("./commands/task_assessment");
...
const { vibeMaintainer } = require("./commands/vibe_maintainer");
...
function run(argv) {
  ...
  // router:sync group === "sync"
  // router:conflict group === "conflict"
  return dispatchCommand({
    ...
    rest,
    rawGroup,
    context: getCommandDispatchContext()
  });
}
...
function task(action, id, flags, rest = []) {
  ...
  if (action === "packet" || action === "packet-preview") {
    const pipelineState = buildPipelineState({ readJsonFile, fileExists });
    assertStrictPipeline("task-packet", pipelineState, { table });
    const { compileTaskControlPlanePacket, renderTaskControlPlanePacket } = kvdfDevBundle;
    const packet = compileTaskControlPlanePacket({
      statuses: flags.statuses || flags.status || "approved,ready",
      evolutionChangeId: flags.evolution || flags.change || evolutionState.current_change_id || null
    }, {
  ...
  if (action === "executor-contract" || action === "contract") {
    ...
    const { compileTaskExecutorContract, renderTaskExecutorContract, renderTaskControlPlanePacket } = kvdfDevBundle;
...
Not specified yet. Describe sections, hierarchy, density, and responsive order.

## Content Requirements

Not specified yet. List visible text, labels, imagery rules, and tone.

## Design Tokens

Not specified yet. Link color, typography, spacing, radius, and motion tokens.

## Responsive Behavior

Not specified yet. Define mobile, tablet, desktop, RTL, and LTR behavior.

## Component States

- loading: Not specified yet.
- empty: Not specified yet.
- error: Not specified yet.
- success: Not specified yet.
- disabled: Not specified yet.

## Data Requirements

Not specified yet. Define required API/data fields, permissions, and empty data handling.

## Accessibility Notes

Not specified yet. Define keyboard, focus, labels, contrast, motion, and screen reader requirements.
...
${page.purpose || "Not specified yet"}

## Audience

${page.audience || "Not specified yet"}

## Layout

Not specified yet. Describe sections, order, density, and hierarchy.

## Data Requirements

Not specified yet. Inputs, API data, states, permissions, and empty data rules.

## Required States

${page.required_states.map((state) => `- ${state}: Not specified yet`).join("\n")}

## Responsive Behavior

Not specified yet. Mobile, tablet, desktop, and RTL/LTR notes.

## Accessibility

Not specified yet. Keyboard, focus, labels, contrast, motion, and screen reader notes.
...
Not specified yet. What the component is for.
...
${component.states.map((state) => `- ${state}: Not specified yet`).join("\n")}

## Props / Inputs

Not specified yet. Expected data, labels, icons, callbacks, and constraints.

## Design Tokens

Not specified yet. Colors, typography, spacing, radius, shadows, and breakpoints.

## Accessibility

Not specified yet. ARIA labels, keyboard behavior, focus order, contrast, and reduced motion.

## Forbidden Variations

Not specified yet. Styles or behaviors the component must not introduce.
...
function getCommandDispatchContext() {
  return {
    doctor,
    resume,
    entryCommand,
