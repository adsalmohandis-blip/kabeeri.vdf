...
- Technical note: If unsure, ask ChatGPT to compare simple CodeIgniter auth options.
...
2. Choose an auth approach suitable for the project: CodeIgniter Shield, session-based auth, JWT/API auth, external provider, or none.
3. Do not choose a paid/external provider without user approval.
4. Add user model/profile foundation only if needed.
5. Add login/logout/register flows only if required.
6. Add password hashing/session/token helpers as appropriate.
7. Add tests or manual check instructions.
8. Do not add complex roles yet; that belongs to the roles/filters prompt.
