...
Configure environment variables, config module, database connection, and local setup rules.
...
- ORM/tool: Prisma, TypeORM, MikroORM, Mongoose, or not sure
...
package.json
README.md
orm/prisma/typeorm files if selected
```
...
4. Add a simple config module or use NestJS config patterns if appropriate.
5. Configure database provider only if selected by the user.
6. Prepare Prisma, TypeORM, MikroORM, or other database tooling only if needed now.
