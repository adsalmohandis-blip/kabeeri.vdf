...
function validateWorkspaceFiles(workspaceRoot, profileOrCategory = "generic") {
  ...
  const pipelineContract = readJson(path.join(workspaceRoot, ".kabeeri", "app_pipeline_contract.json"), null);
  result.pipeline_contract = pipelineContract;
  addCheck("pipeline_contract", Boolean(pipelineContract), pipelineContract ? "pipeline contract parsed." : "pipeline contract missing.", ".kabeeri/app_pipeline_contract.json");
  if (pipelineContract) {
    addCheck("pipeline_contract_app", String(pipelineContract.app && pipelineContract.app.app_slug || "").trim() === path.basename(workspaceRoot), "pipeline contract app slug matches the scaffold root.", ".kabeeri/app_pipeline_contract.json");
    addCheck("pipeline_contract_stage_order", Array.isArray(pipelineContract.stage_order) && pipelineContract.stage_order.join("|") === "app_category_registry|domain_plugin|app_folder_structure", "pipeline stage order is strict.", ".kabeeri/app_pipeline_contract.json");
  }
...
function buildValidationReport(workspaceRoot, validation, metadata = {}) {
  return {
    ...
    pipeline_contract: validation.pipeline_contract || null,
    pipeline_contract_validation: validation.pipeline_contract_validation || null,
