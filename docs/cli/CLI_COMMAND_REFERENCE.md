...
```bash
kvdf planner method --goal "Build secure database app" --track vibe --json
...
The Planner Layer is the deterministic native planning surface for KVDF Core, vibe/app, and plugin work. It reads the current repository and runtime context, recommends the next governed Evolution, chooses a planning method, generates a Task Punch, persists proposed plans under `.kabeeri/planner.json`, can review or resume planner state, can materialize draft documentation files, can generate a Codex-ready prompt from the approved current plan, and can emit a visual planning model with Mermaid, board, scope map, and markdown report output. For Viber/App Track it models `documentation_folders` as the primary docs surface and keeps `documentation_files` as the compatibility flattening view that maps back to the portable numbered app-doc package. KVDF Core defaults to direct-to-main delivery, so the planner should never imply branch/PR as the normal path. Branch and PR remain optional only for team, protected-repo, or risky work. When no track is provided, the owner repo defaults to the owner planner, app work defaults to vibe planner mode, and `--plugin` selects plugin mode.

The shared Roadmap Train and Evo Sprint Queue commands let the planner persist ordered version/stage/sprint/evolution/task queues for owner and Viber tracks so the next session can resume FIFO state from JSON instead of recreating it from chat or generated reports. Train reports also surface a reusable `roadmap_train_summary` and `evo_sprint_queue` view so pipeline, dashboard, visual, prompt, and readiness outputs can point to the same FIFO state without recomputing it from scratch.
...
Use `kvdf planner method` to recommend `structured`, `agile`, or `hybrid` before planning. Use `kvdf planner current-state` to rebuild the live repo/workspace summary, `kvdf planner boundary` to check the target write boundary, and `kvdf planner stale-state` to classify stale roadmap, report, and runtime files before trusting a next step. The current-state summary is file-first: latest main and merged history outrank stale snapshots, and KVDOS is treated as an app workspace rather than KVDF Core. Use `kvdf truth audit` and `kvdf planner truth` to compare source-level implementation evidence against runtime and report claims, `kvdf planner auto` to generate the full self-planning package, `kvdf planner review` to inspect scope, method, docs, security, source control, task quality, and visual readiness, and `kvdf planner resume` to recover the current planning context. Use `kvdf planner docs catalog|plan|materialize|status|apply-stage|review` to plan, draft, stage, and review foldered app documentation without executing or materializing work. The foldered docs layer is stage-driven, while the portable numbered app-doc package remains the canonical durable app knowledge base. Use `kvdf planner version status|next|gate|publish-ready|mark-published` to inspect Viber version readiness, gate health, and recorded published state without publishing anything automatically.

For Viber/App planning, `kvdf planner docs plan` and `kvdf planner docs materialize` can optionally include UI/UX intelligence when `--include-ui-ux-intelligence` or `--ui-ux-intelligence` is passed. `kvdf planner review`, `kvdf planner visual`, and `kvdf planner prompt` can also surface the same optional provider summaries when requested. That optional provider stays offline and deterministic, uses only `plugins/ui_ux_intelligence/data/`, and does not write files unless you are already using the docs materialization command. Use `--no-ui-ux-intelligence` to suppress the provider even when the plugin is available. Tailwind guidance is separate: `kvdf planner docs plan`, `kvdf planner review`, `kvdf planner visual`, and `kvdf planner prompt` can include a read-only Tailwind provider summary when `--include-tailwind-ui` or `--tailwind-ui` is passed, and `--no-tailwind-ui` suppresses it. Tailwind remains guidance-only and never becomes a Core dependency.
The plugin also exposes `patterns`, `implementation-guidance`, `prompt-pack`, `evidence`, `visual-qa`, `acceptance-gate`, and `regression` for task-punch-friendly UI implementation and review guidance, with Markdown export remaining opt-in through `--output <path>`.
...
kvdf ui-ux-intelligence governance --json
  ...
  kvdf ui-dashboard-kits provider
  kvdf ui-dashboard-kits recommend --surface owner-dashboard
  kvdf ui-dashboard-kits html-comment --surface owner-dashboard
...
kvdf plugins uninstall tailwind_ui
```
...
`ui_dashboard_kits` is an optional removable UI dashboard kits plugin that owns the active UI check logic, dashboard examples, starter templates, snippets, provider metadata, and the compatibility path for `npm run ui:check`. It keeps the legacy core UI execution kit as a reference surface only, delegates the checker to `plugins/ui_dashboard_kits/`, and does not require Bootstrap or Tailwind packages in KVDF Core. The plugin is offline, removable, safe to disable without affecting normal Core startup or rendering, and can return dashboard surface recommendations without redesigning anything.
...
`tailwind_ui` is an optional removable Tailwind guidance provider plugin. It keeps `tailwindcss` and `@tailwindcss/cli` out of the KVDF Core dependency graph, and exposes safe status, verify, provider, planner-guidance, docs-guidance, html-comment, snippet, and utility-map helpers for surfaces that explicitly opt in to Tailwind guidance. The plugin is offline, manifest-driven, and guidance-only in this phase. It does not run the Tailwind CLI, does not fetch CDN assets, and when it is unavailable surfaces should fall back to the existing HTML/CSS rendering path.
...
emits resumable execution reports for each priority, provides a governed
batch-exe queue for ready and approved tasks in priority order, checks possible
duplicate capability matches against the central capability reference, auto-assigns
missing EVO assignees to the active Multi-AI leader or codex fallback before queue generation, infers
...
contract, the stage-transition state, and the docs/design gate summary so the
queue cannot jump from raw idea to execution before the documentation
architecture, foldered docs, system design, database design, UI/UX design, and
version plan are ready. When a priority is already
...
`kvdf task assessment` generates a structured assessment before large work starts so scope, blockers, dependencies, and readiness gates are visible. `kvdf task coverage` writes a full-task-coverage report that shows the planned execution slices, the materialized temporary queue, and whether any remainder is still open. `kvdf task lifecycle` renders the visible lifecycle board from intake to ready, execution, validation, closure, and archived states. For Viber/App tasks, `done` / `completed` / `closed` stay in closure until the Viber explicitly asks to archive them, and the JSON and human-readable lifecycle output includes the archive warning and retention metadata. `kvdf task archive` is the explicit archive path and requires confirmation for Viber/App tasks before anything moves to trash. `kvdf task packet` is the control-plane command: it compiles a durable packet from vibe intake, questionnaire answers, briefs, module breakdowns, and approved task state before any executor starts work, and writes the packet to `docs/reports/KVDF_TASK_CONTROL_PLANE_PACKET.json`. `kvdf task executor-contract` is the executor-boundary command: it turns that packet into a packet-only boundary report that lists allowed files, allowed actions, forbidden actions, and the exact next-command hints for the AI worker. `kvdf task batch-run` is the plugin-owned framework execution helper: it walks approved or ready tasks in governed priority order, auto-assigns missing tasks to the active Multi-AI leader or `codex` fallback, and in `--mode dry-run` / `--mode review` can preview the packet or packet-plus-contract without starting tasks. In `--mode execute` it starts each task until a blocker appears, then writes a durable report to `.kabeeri/reports/task_batch_run.json`. `kvdf task status` shows a single task with its lifecycle stage and next action, and reads from trash when a task is no longer active. `kvdf task complete`, `close`, `finish`, and `archive` move an owner-verified task into `.kabeeri/task_trash.json` when archiving is explicitly requested; for Viber/App tasks, closure is the default terminal state and trashing requires confirmation. The trash retains the full task payload plus the trashed timestamp, expiry timestamp, reason, actor, original position, retention metadata, retention_days, and a warning that trash is recoverable only until retention expires (default 30 days). `kvdf task trash restore <task-id>` returns the task to the active list, and `kvdf task trash purge` / `kvdf task trash sweep` remove expired trash records.

`kvdf maintenance` is an alias for the repo-wide cleanup workflow, and it supports `fast` and `slow` modes. Use `kvdf maintenance fast` for a quicker, lighter audit and `kvdf maintenance slow` for the strict evidence-heavy pass. `kvdf cleaner relocate review` is the strict relocation review mode for the cleanup workflow. It lists the file-by-file evidence for each proposed folder move before any change is applied, and it accepts `--threshold 0.9` to hide weaker candidates. `kvdf cleaner relocate` previews or applies safe file and folder moves when an alias-root path is mislocated, merges alias-root folders into their canonical target folders when needed, and saves a durable relocation report alongside the audit and inspection reports.
...
`kvdf session replay` rebuilds the last session line of work from `.kabeeri/sessions.json`, the saved trace file, the task memory block, and the archive state. It writes `.kabeeri/reports/<session-id>.replay.json`, or prints the replay object directly when `--json` is used. This is the preferred way to continue a session without rereading the original chat.
...
The shared planner cycle feeds both dashboards, but the rendered dashboard product stays separated by track. Planner and pipeline summaries appear in the dashboard JSON and HTML for the active track only. When available, the Viber dashboard can also surface an optional `ui_ux_intelligence` summary for the current app-planning context without making the plugin mandatory.
...
See `docs/reports/dashboard/LIVE_DASHBOARD_RUNTIME.md`.
