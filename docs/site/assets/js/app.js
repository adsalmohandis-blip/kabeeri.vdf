const pages = [
  ...
  ["ai-tool", "AI Tool Instructions", "تعليمات أداة الذكاء الاصطناعي"],
  ["vibe-developer", "Vibe Developer Instructions", "تعليمات مطور Vibe"],
  ["vibe-developer-pipeline", "Vibe Developer Pipeline", "مسار مطور Vibe"],
  ["ai-tool-hub", "AI Tool Docs Site", "بوابة أداة AI"],
  ["vibe-developer-hub", "Vibe Developer Docs Site", "بوابة مطور Vibe"],
...
const roleNavScopes = {
  "ai-tool": { label: { en: "AI Tool Surface", ar: "سطح أداة AI" }, pages: ["ai-tool-hub", "ai-tool"] },
  "ai-tool-hub": { label: { en: "AI Tool Surface", ar: "سطح أداة AI" }, pages: ["ai-tool-hub", "ai-tool"] },
  "vibe-developer": { label: { en: "Vibe Developer Surface", ar: "سطح مطور Vibe" }, pages: ["vibe-developer-hub", "vibe-developer", "vibe-developer-pipeline"] },
  "vibe-developer-hub": { label: { en: "Vibe Developer Surface", ar: "سطح مطور Vibe" }, pages: ["vibe-developer-hub", "vibe-developer", "vibe-developer-pipeline"] },
  "vibe-developer-pipeline": { label: { en: "Vibe Developer Surface", ar: "سطح مطور Vibe" }, pages: ["vibe-developer-hub", "vibe-developer", "vibe-developer-pipeline"] }
};
...
const capabilityRows = {
  en: [
    ["CLI Engine", "Runs local workspace operations, validation, task lifecycle, dashboard export, package checks, and release gates. Keep the command layer small and let live reports and human docs carry the longer narrative.", "`bin/kvdf.js`, `src/cli/`, `docs/cli/CLI_COMMAND_REFERENCE.md`"],
    ...
    ["Release Readiness", "Checks validation, policy, notes, checklist, and publish gates before release. Release docs should point to exact state, not only summaries.", "`kvdf release`, `docs/production/`"],
    ["Reports And Traceability", "Keeps audit, gap, implementation, requirement, and validation reports aligned to the current source of truth.", "`docs/reports/`"],
  ...
  ar: [
    ["محرك CLI", "يشغل عمليات المساحة المحلية والتحقق ودورة حياة التاسكات وتصدير الداشبورد وفحص التغليف وبوابات الإصدار. اجعل طبقة الأوامر صغيرة ودع التقارير الحية والدوكس البشرية تحمل الشرح الطويل.", "`bin/kvdf.js`, `src/cli/`, `docs/cli/CLI_COMMAND_REFERENCE.md`"],
    ...
    ["جاهزية الإصدار", "تفحص validation والسياسات والملاحظات وقائمة التحقق وبوابات النشر قبل الإصدار. يجب أن تشير وثائق الإصدار إلى الحالة الدقيقة لا إلى الملخص فقط.", "`kvdf release`, `docs/production/`"],
    ["التقارير والتتبع", "تحافظ على تقارير audit وgap وimplementation وrequirements وvalidation مرتبطة بمصدر الحقيقة الحالي.", "`docs/reports/`"],
...
const docs = {
  en: {
    ...
    pages: {
      "what-is": {
        lead: "Kabeeri VDF is a local-first operating framework for AI-assisted software development. It works with any AI coding assistant or automation tool the developer chooses. It does not replace Laravel, React, Next.js, Django, .NET, WordPress, hosting, Git, or the AI tool itself; it governs how a software idea becomes answers, tasks, prompts, implementation, review, dashboards, cost records, final verification, and release decisions. Before a write-capable planning step, Kabeeri now expects a file-first current-state rebuild, workspace boundary check, and truth reconciliation so stale roadmap text does not outrank the live repo.",
        beginner: "Think of Kabeeri as the project memory, delivery rulebook, AI coordination layer, and live dashboard around your codebase. The application is still built with its normal stack; Kabeeri keeps the work scoped, traceable, reviewable, resumable, and honest about what is already implemented.",
        sections: [
          ["What it governs", "Project intake, delivery mode, current-state rebuilds, application boundaries, workstreams, questionnaires, product blueprints, data design, UI/UX guidance, prompt packs, tasks, reviews, policies, dashboards, GitHub sync, AI usage, releases, and stale-state detection."],
          ["State resync and truth", "Planner current-state, boundary, stale-state, truth, roadmap train, and version readiness checks run before any new write-capable planning step."],
          ["Role instructions", "Use the AI Tool instructions and Vibe Developer instructions as the working agreement for planning and app delivery."],
        ...
        featuredPages: [
          ["ai-tool-hub", "AI Tool Docs Site", "Entry point for the AI Tool role surface."],
          ["vibe-developer-hub", "Vibe Developer Docs Site", "Entry point for app-builder development."]
        ],
      ...
      "start-here": {
        ...
        sections: [
          ["Scenario 1: new application", "Use product blueprint, delivery mode, questionnaire plan, review and approval, data design, UI/UX advisor, prompt packs, and task governance before implementation."],
        ...
        steps: ["Identify scenario", "Validate", "Read dashboard", "Plan questions", "Review planning pack", "Approve planning pack", "Select Agile or Structured", "Create tasks", "Use prompt pack", "Record evidence", "Final verify"],
        details: [
        ...
        checklist: ["You know which scenario you are in.", "Validation is green or blockers are known.", "The planning pack exists and is approved.", "The next task has source and acceptance criteria.", "The selected prompt pack matches the stack.", "Any broad work has a capture or planning record."],
        commands: ["node bin/kvdf.js validate", "node bin/kvdf.js questionnaire plan", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js delivery recommend", "node bin/kvdf.js task tracker --json"]
      },
      "install-profiles": {
      ...
      "ai-with-kabeeri": {
        ...
        sections: [
          ...
          ["Role guides", "The AI Tool page explains how to behave inside a task. The System Development docs site explains how to change the framework. The Vibe Developer page explains how to build an app inside a workspace."],
      ...
      "capabilities": {
        lead: "This page is the web version of the Kabeeri capabilities reference. It explains what each capability owns, why it exists, where a developer should look, and how AI should stay inside the correct owner/app/plugin lane while using the CLI directly. It also shows when a capability is governed by a packet, a bundle contract, or another explicit source of truth.",
        beginner: "When you are lost, this is the map. Pick the capability that matches your problem, then follow its source files and commands. If the work already has state, let the AI use the CLI directly instead of rebuilding the context in chat. If a capability is delivered by a removable bundle, read the bundle contract first.",
        ...
        sections: [
          ...
          ["Control-plane boundaries", "Some capabilities are executed through a packet-first workflow. The packet captures the task, the executor contract narrows the allowed files and actions, and batch-run only starts approved work in governed order."],
          ["Historical reports", "Historical reports explain why a state changed. Live JSON reports show the current execution state, so if the two disagree you update the source state and regenerate the derived surface."],
          ["Bundle contracts", "Some capabilities are delivered by removable bundles. Their manifest and folder contract define the source of truth, and the loader should surface the next exact action instead of making the operator infer it from chat history."],
          ...
          ["How capabilities connect", "Product Blueprints feed Questionnaires. Questionnaires feed Tasks. Data Design and UI/UX Advisor improve those tasks. Prompt Packs guide AI implementation. Dashboard, policies, cost, current-state, truth, and final verification close the loop."]
        ],
        ...
        details: [
          ...
          ["Maintenance rule", ["When a capability changes, update the source docs, runtime implementation, schema if any, tests, and this docs site.", "The capabilities reference should remain the executive index of the whole framework, including the owner/app split, plugin-first guidance, the packet workflow, the live report rule, and the ability to add or remove feature bundles cleanly."]]
        ],
        checklist: ["Capability exists in docs.", "Implementation or runtime state exists when needed.", "CLI or validation path exists when needed.", "Dashboard or reports can expose important state.", "The capability is reflected in this site.", "The owner/app/plugin lane is clear for AI and humans.", "Feature bundles can be added or removed without ambiguity.", "Packet-first boundaries are documented when the capability uses them."]
      },
      "repository-layout": {
        ...
        sections: [
          ...
          ["`workspaces/`", "Developer app workspaces that live under `workspaces/apps/<app-slug>/` with local app `.kabeeri` state, tests, docs, package metadata, and a strict validation contract."],
      ...
      "new-project": {
        ...
        commands: ["node bin/kvdf.js init --profile standard --mode agile", "node bin/kvdf.js dashboard generate", "node bin/kvdf.js vibe \"I want to build an ecommerce store\"", "node bin/kvdf.js validate", "node bin/kvdf.js track status", "node bin/kvdf.js questionnaire plan \"Build an ecommerce store\"", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js blueprint recommend \"Build an ecommerce store\"", "node bin/kvdf.js data-design context ecommerce", "node bin/kvdf.js design recommend ecommerce", "node bin/kvdf.js prompt-pack list"]
      },
      ...
      "existing-non-kabeeri-project": {
        ...
        commands: ["node bin/kvdf.js init --profile standard --mode structured", "node bin/kvdf.js project analyze --path .", "node bin/kvdf.js validate", "node bin/kvdf.js app list", "node bin/kvdf.js workstream list", "node bin/kvdf.js questionnaire plan \"Adopt existing application\"", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js adr list"]
      },
      "delivery-mode": {
      ...
      "questionnaire-engine": {
        ...
        commands: ["node bin/kvdf.js questionnaire plan", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js questionnaire coverage", "node bin/kvdf.js questionnaire missing"]
      },
      "product-blueprints": {
        lead: "Product Blueprints teach Kabeeri common market application patterns so AI planning starts from a professional baseline instead of a blank page. The booking and ecommerce business-type packs stay in parity so related archetypes share the same guidance shape.",
        beginner: "When a developer says ecommerce, POS, ERP, CRM, blog, news, booking, delivery, or mobile app, Kabeeri should already know the usual channels, modules, pages, database areas, workstreams, and risks. Booking and ecommerce are paired business-type packs, so their documentation should evolve together.",
        sections: [
          ...
          ["Business-type parity", "Booking and ecommerce use matching pack structure, so archetype guidance, intake themes, module progression, and acceptance style stay aligned."],
        ...
        details: [
          ...
          ["Parity rule", ["When a booking or ecommerce pack changes, update both sides together unless the change is intentionally unique to one archetype.", "That keeps the shared business-type guidance coherent for humans and AI."]]
        ],
        checklist: ["Product type is selected.", "Channels are known.", "Modules are accepted or deferred.", "Database areas are identified.", "Risks are listed.", "Booking and ecommerce guidance stays in parity when both packs exist."],
      ...
      "task-governance": {
        ...
        sections: [
          ...
          ["Control plane", "The task packet compiles the durable task snapshot, the executor contract narrows the worker boundary, and the batch runner starts only approved work in governed order."],
        ...
        details: [
          ...
          ["Task packet flow", ["Create the packet first, derive the executor contract second, then use batch-run only for approved tasks.", "That sequence keeps the AI boundary explicit and prevents execution from drifting ahead of the accepted plan."]],
          ...
          ["Archive trail", ["Completed tasks move into task trash instead of disappearing.", "The task scheduler keeps route history for trash, restore, temp, agent handoff, and deferred movement so later sessions can reconstruct what happened without chat memory."]],
        ...
        checklist: ["Task has source.", "Acceptance criteria are testable.", "Allowed files are clear.", "Reviewer exists.", "Evidence is recorded.", "Final decision is recorded when required.", "Packet, executor contract, and batch-run are documented when used.", "Archive trail is visible after completion and trash routing."],
      ...
      "practical-examples": {
        ...
        commands: ["node bin/kvdf.js blueprint recommend \"Build a supermarket POS\"", "node bin/kvdf.js questionnaire plan \"Build a dental clinic booking system\"", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js data-design context ecommerce", "node bin/kvdf.js design recommend ecommerce", "node bin/kvdf.js prompt-pack compose react-native-expo --task task-mobile-001", "kvdf wordpress plan \"Build a WordPress digital marketing agency website with services blog case studies and lead forms\" --type corporate --mode new", "node bin/kvdf.js handoff package --id client-handoff --audience client"]
      },
      "example-ecommerce": {
        ...
        commands: ["node bin/kvdf.js blueprint recommend \"Build a complete ecommerce store with payments shipping admin and SEO\"", "node bin/kvdf.js questionnaire plan \"Build ecommerce store with catalog cart checkout payment shipping admin\"", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js data-design context ecommerce", "node bin/kvdf.js design recommend ecommerce", "node bin/kvdf.js prompt-pack compose laravel --task task-backend-001", "node bin/kvdf.js prompt-pack compose react --task task-storefront-001"]
      },
      ...
      "example-blog": {
        ...
        commands: ["node bin/kvdf.js blueprint recommend \"Build a personal blog with SEO newsletter and articles\"", "node bin/kvdf.js questionnaire plan \"Personal blog with articles tags categories and newsletter\"", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js design recommend blog_website", "node bin/kvdf.js prompt-pack compose astro --task task-blog-001"]
      },
      ...
      "example-dental-clinic": {
        ...
        commands: ["node bin/kvdf.js blueprint recommend \"Dental clinic appointment booking website with admin calendar\"", "node bin/kvdf.js questionnaire plan \"Dental clinic booking system with dentists services patients reminders\"", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js data-design context booking_appointment", "node bin/kvdf.js design recommend booking_system"]
      },
      "example-crm": {
        ...
        commands: ["node bin/kvdf.js blueprint recommend \"Professional CRM with leads contacts deals pipeline reports\"", "node bin/kvdf.js questionnaire plan \"CRM with sales pipeline activities import export and reporting\"", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js data-design context crm", "node bin/kvdf.js design recommend crm"]
      },
      "example-mobile-commerce": {
        ...
        commands: ["node bin/kvdf.js app create --username store-mobile --name \"Store Mobile App\" --type mobile --path apps/mobile --product Store", "node bin/kvdf.js questionnaire plan \"Mobile app for ecommerce store with checkout orders and push notifications\"", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js design recommend ecommerce", "node bin/kvdf.js prompt-pack compose react-native-expo --task task-mobile-001"]
      },
      "example-pos": {
    ...
    pages: {
      "what-is": {
        ...
        sections: [
          ...
          ["تعليمات الأدوار", "استخدم صفحات تعليمات أداة AI ومطور Vibe كاتفاق عمل للتخطيط وتسليم التطبيقات."],
        ...
        featuredPages: [
          ["ai-tool-hub", "بوابة أداة AI", "نقطة الدخول لسطح أداة AI."],
          ["vibe-developer-hub", "بوابة مطور Vibe", "نقطة الدخول لبناء التطبيقات."]
        ],
...
const arOverrides = {
  "start-here": ["ابدأ من هنا", "هذه خريطة الدخول لأي مطور يفتح فولدر كبيري ويريد أن يعرف ماذا يفعل بدون تخمين.", "ابدأ بتحديد السيناريو: تطبيق جديد، مشروع كبيري قائم، أو كود موجود لم يبنَ بكبيري. في التطبيق الجديد، أنشئ planning pack ثم راجعه واعتمده قبل التاسكات. كل سيناريو له طريق آمن مختلف."],
  ...
  "new-project": ["بدء تطبيق جديد", "استخدم هذا المسار عندما تريد بناء تطبيق جديد من فكرة أو طلب عميل أو وصف طبيعي.", "لا تقفز للكود مباشرة. اجعل كبيري يفهم نوع المنتج ونمط التسليم وحدود التطبيقات وتصميم البيانات واتجاه الواجهة وخطة التاسكات، ثم أنشئ planning pack وراجعه واعتمده قبل التنفيذ."],
...
const arDeepOverrides = {
  "ai-with-kabeeri": {
    sections: [
      ...
      ["أدلة الأدوار", "صفحة أداة AI تشرح كيف تتصرف داخل التاسك. صفحة مطور كبيري تشرح كيف تغيّر الفريمورك. صفحة مطور Vibe تشرح كيف تبني تطبيقاً داخل مساحة العمل."],
  ...
  "capabilities": {
    lead: "هذه الصفحة هي النسخة الويب من مرجع قدرات كبيري. تشرح ماذا تملك كل قدرة ولماذا موجودة وأين يبحث المطور وكيف يبقى AI داخل المسار الصحيح للمالك أو التطبيق أو البلغن. وتوضح أيضًا متى تكون القدرة محكومة بحزمة أو بعقد فولدرات أو بمصدر حقيقة آخر، ومتى تكون التقارير الحية هي واجهة التنفيذ والتقارير التاريخية هي واجهة الشرح.",
    beginner: "عندما تضيع، هذه هي الخريطة. اختر القدرة الأقرب للمشكلة ثم اتبع ملفاتها وأوامرها. وإذا كانت القدرة تأتي من bundle قابل للإزالة فاقرأ عقده أولًا. وإذا تعارضت التقارير، صحح حالة المصدر ثم أعد توليد المخرج المشتق.",
    sections: [
      ["كيف تقرأ الجدول", "كل صف يمثل قدرة داخل كبيري: ماذا تفعل، أين تعيش، وكيف تساعد المطور أو AI."],
      ["متى تكون القدرة محكومة", "بعض القدرات تُنفذ عبر packet أولًا ثم executor contract ثم batch-run. هذا يفصل التخطيط عن التنفيذ."],
      ["التقارير الحية", "التقارير الحية JSON تعرض الحالة التنفيذية الحالية، أما التقارير التاريخية فتشرح كيف وصلنا إليها. إذا حصل تعارض، صحح حالة المصدر وأعد التوليد بدل الاعتماد على التقرير التاريخي كسجل runtime."],
      ["عقود الباندل", "بعض القدرات تأتي من bundles قابلة للإزالة. الـ manifest وعقد الفولدرات هما مصدر الحقيقة، والloader يجب أن يعرض الخطوة التالية بوضوح."],
      ...
      ["كيف تحافظ عليها", "أي تغيير في القدرة يجب أن ينعكس في الدوكس والتنفيذ والاختبارات والداشبورد إن لزم. وطبقة الأوامر يجب أن تبقى مختصرة بينما تحمل التقارير الحية والدوكس البشرية الشرح الأطول."]
    ],
    details: [
    ...
    checklist: ["القدرة موثقة.", "لها تنفيذ أو runtime عند الحاجة.", "لها علاقة واضحة بالتاسكات أو الداشبورد أو AI.", "موجودة في مرجع القدرات.", "عقود الحزم والفولدرات موثقة عندما تكون القدرة قابلة للإزالة."]
  },
  "repository-layout": {
    sections: [
  ...
  "new-project": {
    sections: [
      ...
      ["ابدأ نظام الأسئلة", "استخدم questionnaire plan لإنشاء planning pack، ثم questionnaire review، ثم questionnaire approve --confirm قبل أي تاسكات، لاكتشاف المنتج والباك إند والفرونت إند والبيانات والواجهة والمخاطر."],
      ["أكمل التطوير", "استخدم blueprint وdata design وUI/UX وprompt packs والتاسكات والداشبورد للتنفيذ خطوة بخطوة بعد اعتماد planning pack."]
    ],
    details: [
  ...
  "product-blueprints": {
    sections: [
      ...
      ["Parity rule", "حزما booking وecommerce يجب أن تظل متوازيتين في ترتيب العناوين، وأنواع الإدخال، ومسار الموديولات، ونمط القبول."],
    ...
    details: [
      ...
      ["قاعدة التماثل", ["إذا تغيرت حزمة booking أو ecommerce، حدث الأخرى أيضًا إلا إذا كان التغيير خاصًا بأحدهما فقط.", "هذا يحافظ على اتساق الدليل المشترك بين الحجز والتجارة."]]
    ],
    checklist: ["نوع المنتج محدد.", "القنوات معروفة.", "الموديولات مقبولة أو مؤجلة.", "مناطق قاعدة البيانات محددة.", "المخاطر ظاهرة.", "تماثل booking وecommerce محفوظ عندما تكون الحزمتان موجودتين."]
  },
  "data-design": {
    sections: [
  ...
  "task-governance": {
    ...
    sections: [
      ...
      ["عقد التحكم", "الـ task packet يجمع حالة التاسك الدائمة، والـ executor contract يضيق حدود worker، والـ batch-run يبدأ فقط العمل المعتمد وبالترتيب الحاكم."],
    ...
    details: [
      ...
      ["مسار packet", ["أنشئ packet أولًا، ثم derive executor contract ثانيًا، ثم استخدم batch-run فقط للعمل المعتمد.", "هذا يبقي حد AI واضحًا ويمنع التنفيذ من تجاوز الخطة المقبولة."]],
      ["Live JSON", ["حالة task tracker يمكن قراءتها من الداشبورد وVS Code.", "هذا يقلل تعديل ملفات توثيق بعد كل تاسك."]],
      ["Archive trail", ["التاسكات المكتملة تنتقل إلى task trash بدل الاختفاء.", "Task scheduler يحتفظ بتاريخ الحركة إلى trash وrestore وtemp وagent handoff وdeferred حتى يمكن استرجاع المسار لاحقًا بدون chat memory."]],
    ...
    checklist: ["التاسك له مصدر.", "القبول قابل للاختبار.", "الملفات المسموحة واضحة.", "الدليل مسجل.", "قرار المالك مسجل عند الحاجة.", "packet وexecutor contract وbatch-run موثقين عندما يُستخدمون.", "archive trail ظاهر بعد الإنهاء ونقل المهمة للtrash."]
  },
  "app-boundary": {
    sections: [
...
docs.en.pages["developer-onboarding"] = {
  lead: "This page gives a first-session path for developers who need to enter a Kabeeri workspace safely, understand the track, and follow the planning gate without guessing.",
  beginner: "Use this when you are new to the folder or returning after a break. The opening sequence is strict: identify the track, confirm the workspace, create the planning pack, review it, approve it, then move into the first safe task.",
  sections: [
    ...
    ["App developer onboarding", "Run resume, confirm the app track, create the planning pack, review it, approve it, validate the app workspace contract, inspect the task tracker, and use vibe or task commands only inside the app workspace."],
    ["New workspace onboarding", "If the folder is not yet a Kabeeri workspace, initialize it first, then return to resume, create the planning pack, review it, approve it, and confirm the track before any implementation."],
    ["The opening rule", "The first command should tell you where you are, what you are allowed to touch, and what exact step comes next. If it does not, keep reading state instead of editing code. No implementation task should exist before the planning pack is reviewed and approved."]
  ],
  steps: ["Run resume", "Confirm the track", "Create the planning pack", "Review it", "Approve it", "Read the docs or dashboard", "Inspect the next safe command", "Open the first task", "Implement one slice", "Record evidence"],
  details: [
    ["What to do in the first ten minutes", ["Read the current state, check the repository map, create the planning pack, review it, approve it, open the docs manifest, and inspect the top-level task or evolution queue.", "Do not start a broad refactor just because the folder is familiar. The safe path is always smaller than it feels."]],
    ["How this reduces mistakes", ["Onboarding keeps the first session short, which reduces accidental edits, wrong-track commands, and repeated context hunting.", "The point is not speed alone. The point is a clean restart that can be repeated tomorrow by another developer or AI agent with the planning gate intact."]]
  ],
  checklist: ["The current track is known.", "The workspace mode is known.", "The planning pack exists.", "The planning pack is reviewed and approved.", "The next exact command is visible.", "The first task or priority is scoped.", "The developer can resume without chat history."],
  commands: ["node bin/kvdf.js resume --json", "node bin/kvdf.js onboarding", "node bin/kvdf.js questionnaire plan", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js app workspace validate", "node bin/kvdf.js docs manifest", "node bin/kvdf.js dashboard state"]
};

docs.ar.pages["developer-onboarding"] = {
  ...docs.en.pages["developer-onboarding"],
  arTitle: "تأهيل المطور",
  lead: "هذه الصفحة تعطي مسار الجلسة الأولى للمطور الذي يدخل مساحة Kabeeri بأمان، يفهم المسار، ويلتزم ببوابة التخطيط بدون تخمين.",
  beginner: "استخدمها عندما تكون جديدًا على الفولدر أو عائدًا بعد انقطاع. المسار صارم: حدّد track، تأكد من مساحة العمل، أنشئ planning pack، راجعه، اعتمده، ثم انتقل إلى أول تاسك آمن.",
  sections: [
    ["تهيئة صاحب المشروع", "شغّل resume، راجع أولوية Evolution النشطة، تأكد من الـ temporary queue، وأنشئ planning pack إذا كان العمل الجديد يحتاجه، ثم اتبع الـ slice الحالي قبل أي تغيير في الـ framework."],
    ["تهيئة مطور التطبيق", "شغّل resume، تأكد من track الخاص بالتطبيق، وأنشئ planning pack، ثم راجعه واعتمده قبل task tracker أو أي تنفيذ داخل مساحة التطبيق."],
    ["تهيئة مساحة جديدة", "إذا لم تكن المجلدات مهيأة بعد كمساحة Kabeeri، نفّذ init أولًا، ثم عد إلى resume، أنشئ planning pack، راجعه واعتمده، ثم أكد المسار قبل أي تنفيذ."],
    ["قاعدة البداية", "أول أمر يجب أن يوضح أين أنت، وما المسموح لك لمسه، وما الخطوة الدقيقة التالية. إذا لم يفعل ذلك، استمر في قراءة الحالة بدل تعديل الكود. لا يجوز أن يوجد أي implementation task قبل مراجعة واعتماد planning pack."]
  ],
  steps: ["شغّل resume", "أكد المسار", "أنشئ planning pack", "راجعه", "اعتمده", "اقرأ docs أو dashboard", "راجع الأمر الآمن التالي", "افتح أول تاسك", "نفذ slice واحد", "سجل الأدلة"],
  details: [
    ["ماذا تفعل في أول عشر دقائق", ["اقرأ الحالة الحالية، راجع خريطة المستودع، أنشئ planning pack، راجعه واعتمده، افتح manifest الخاص بالـ docs، وراجع أعلى task أو evolution queue.", "لا تبدأ refactor واسع فقط لأنك تعرف المجلد. المسار الآمن دائمًا أصغر مما يبدو."]],
    ["كيف يقلل الأخطاء", ["التأهيل يبقي الجلسة الأولى قصيرة، فيقلل التعديلات الخطأ وأوامر track الخاطئة والبحث المتكرر عن السياق.", "الهدف ليس السرعة فقط، بل إعادة بدء نظيفة يمكن تكرارها غدًا بواسطة مطور آخر أو وكيل AI مع بوابة التخطيط نفسها."]]
  ],
  checklist: ["المسار الحالي معروف.", "وضع مساحة العمل معروف.", "planning pack موجود.", "planning pack راجع ومُعتمد.", "الأمر الدقيق التالي ظاهر.", "أول task أو priority محدد النطاق.", "المطور يستطيع الاستئناف بدون ذاكرة chat."],
  commands: ["node bin/kvdf.js resume --json", "node bin/kvdf.js onboarding", "node bin/kvdf.js questionnaire plan", "node bin/kvdf.js questionnaire review", "node bin/kvdf.js questionnaire approve --confirm", "node bin/kvdf.js docs manifest", "node bin/kvdf.js dashboard state"]
};
...
const pageCommandCatalog = {
  ...
  "start-here": [
    ...
    c("node bin/kvdf.js questionnaire plan \"Describe the app you want\"", "Generate the first planning pack for a new or unclear product idea, then review and approve it before tasks.", "ينشئ أول planning pack لفكرة التطبيق ثم يراجع ويعتمد قبل التاسكات."),
  ...
  "new-project": [
    ...
    c("node bin/kvdf.js questionnaire plan \"Build an ecommerce store\"", "Generate the first planning pack for the project, then review and approve it before task generation.", "ينشئ أول planning pack للمشروع ثم يراجع ويعتمد قبل توليد التاسكات."),
  ...
  "questionnaire-engine": [
    ...
    c("node bin/kvdf.js questionnaire plan \"Build CRM\"", "Generate an adaptive planning pack from the product description and keep it pending until review and approval.", "ينشئ planning pack تكيفيًا من وصف المنتج ويظل pending حتى المراجعة والاعتماد."),
    ...
    c("node bin/kvdf.js questionnaire generate-tasks", "Generate proposed tasks only after the questionnaire planning pack has been reviewed and approved.", "يولد تاسكات مقترحة فقط بعد مراجعة واعتماد planning pack."),
  ...
  "practical-examples": [
    ...
    c("node bin/kvdf.js questionnaire plan \"Build a dental clinic booking system\"", "Generate the planning pack for the selected example, then review and approve it before tasks.", "ينشئ planning pack للمثال المختار ثم يراجع ويعتمد قبل التاسكات."),
...
function vibeFirstRow(command, description, lang) {
  ...
  const rows = [
    ...
    [/questionnaire plan/, isAr ? `أنشئ planning pack للأسئلة المهمة فقط لفهم المشروع${subject ? `: ${subject}` : ""}.` : `Create a planning pack for only the important questions needed to understand the project${subject ? `: ${subject}` : ""}.`, isAr ? "يولد planning pack مختصرًا مرتبطًا بالمنتج والفريمورك وقاعدة البيانات والواجهة ثم يبقى معلقًا للمراجعة والاعتماد." : "Generates a compact planning pack linked to product, framework, data, and UI decisions, then keeps it pending review and approval."],
...
function describeCommand(command, lang) {
  ...
  if (text.includes("questionnaire plan")) return isAr ? "ينشئ planning pack للأسئلة ويجب مراجعته واعتماده قبل التاسكات." : "Generate a planning pack for intake questions and review/approve it before tasks.";
...
function vibeFirstRow(command, description, lang) {
  ...
  if (lower.includes("docs open") || lower.includes("docs serve")) {
    return row(
      ...
      "Kabeeri regenerates the docs site shell, serves it locally, and can open it in the browser while the detailed content continues to come from the shared docs source-of-truth workflow.",
      "كبيري يعيد توليد هيكل موقع الوثائق ويشغله محليًا ويمكنه فتحه في المتصفح بينما يأتي المحتوى التفصيلي من سير عمل مصدر الحقيقة المشترك."
    );
  }
  if (lower.includes("docs validate")) {
    return row(
      ...
      "Kabeeri compares the docs site, generated page contracts, and localized guidance against the current command surface so generated pages stay aligned with the human docs workflow.",
      "كبيري يقارن موقع الوثائق وعقود الصفحات المولدة والإرشادات المحلية بسطح الأوامر الحالي حتى تظل الصفحات المولدة متطابقة مع سير عمل التوثيق البشري."
    );
  }
  if (lower.includes("questionnaire plan")) {
    return row(
      `Create the planning pack for this project${subject ? `: ${subject}` : ""} and keep it review/approval gated.`,
      `أنشئ planning pack لهذا المشروع${subject ? `: ${subject}` : ""} مع حجزه للمراجعة والاعتماد.`,
      "Kabeeri generates focused intake questions from product blueprint, delivery mode, data design, UI, and prompt-pack signals, then requires explicit review and approval before tasks.",
      "كبيري يولد أسئلة مركزة من خريطة المنتج ونمط التسليم وتصميم البيانات والواجهة وحزم البرومبت، ثم يفرض مراجعة واعتمادًا صريحين قبل التاسكات."
    );
  }
...
Object.assign(docs.en.pages, {
  "ai-tool": {
    lead: "The AI Tool is the assistant role contract. It can think, summarize, propose, and implement only inside an approved task. It does not approve its own plan, and it does not expand scope silently.",
    beginner: "Use this page when you are working with Codex or another AI tool inside Kabeeri. The rule is simple: plan first, split into tasks, wait for approval, then implement one task at a time and stop.",
    sections: [
      ["Shared rule", "Do not edit files until the Evo or task has been split into explicit steps with dependencies and acceptance criteria."],
      ["What the AI Tool may do", "Read the approved plan, help refine task slices, implement the current task, summarize evidence, and stop when the task is complete."],
      ["What the AI Tool may not do", "It may not bundle unrelated changes, bypass approval, invent missing scope, or continue after the current task without asking."],
      ["Best practice", "Treat every task as a small governed slice. If a change feels bigger than one slice, return to planning before editing."]
    ],
    steps: ["Read the plan", "Confirm scope", "Implement one task", "Verify output", "Stop and wait"],
    details: [
      ["Stop rules", ["If scope changes, pause immediately and ask for approval.", "If the next action is unclear, do not guess."]],
      ["Success definition", ["The AI Tool is successful when it stays within one approved task, leaves a clear trail, and never turns planning into hidden implementation."]]
    ],
    checklist: ["Planning was split into tasks.", "Acceptance criteria are written.", "Approval exists before edits.", "Only one task is active.", "Verification happened before continuing."],
    commands: ["knowledge/governance/KVDF_WORKFLOW_INSTRUCTIONS.md", "docs/reports/KVDF_WORKSPACE_BOUNDARY_IMPLEMENTATION_CHECKLIST.md"]
  },
  "ai-tool-hub": {
    lead: "This is the AI Tool entry surface. It points the assistant toward the strict workflow contract and the approved-task rule before any implementation starts.",
    beginner: "Start here if you are using Codex or another AI tool. This hub tells you where the AI Tool instructions live and how to move from planning to one-task execution without guessing.",
    sections: [
      ["Primary guide", "The AI Tool role page explains how the assistant should behave: plan first, stop after each task, and never expand scope silently."],
      ["Use it when", "You need a quick entry point to the AI Tool instructions, the shared workflow policy, or the role-specific stop rules."],
      ["Related role pages", "From here you can jump to the system development docs site or the Vibe Developer docs surface if the work shifts roles."]
    ],
    featuredPages: [
      ["ai-tool", "AI Tool Instructions", "The strict operating contract for Codex and other AI tools."],
      ["developer-onboarding", "Developer Onboarding", "The standard onboarding path for the Kabeeri workflow and its planning gate."],
      ["what-is", "Overview", "The overview of how Kabeeri works and where the role surfaces fit."]
    ],
    steps: ["Open the AI Tool instructions", "Read the workflow policy", "Check the task slice", "Implement one approved task", "Stop and wait"],
    details: [
      ["Canonical links", ["`knowledge/governance/KVDF_WORKFLOW_INSTRUCTIONS.md`", "`docs/site/pages/en/ai-tool.html`", "`docs/site/pages/en/vibe-developer.html`"]]
    ],
    checklist: ["The assistant has a single approved task.", "The planning pack is approved.", "The workflow policy is available.", "Scope changes trigger a pause."],
  ...
  "vibe-developer-hub": {
    ...
    featuredPages: [
      ...
      ["vibe-developer-pipeline", "Vibe Developer Pipeline", "The end-to-end path from idea to a fully ready app workspace."],
      ["new-project", "Start a New Application", "The strict planning and approval path for new app workspaces."],
      ["questionnaire-engine", "Questionnaire Engine", "The intake and planning layer that produces the approved task-ready plan."],
      ["data-design", "Data Design", "The data modeling guidance for app workspaces and surface scopes."],
      ["task-governance", "Task Governance", "How approved tasks are created, tracked, and completed."],
      ["app-boundary", "App Boundary Governance", "How workspace boundaries protect shared and surface-specific work."],
      ["dashboard-monitoring", "Live Dashboard", "The app workspace dashboard that shows readiness, blockers, and next actions."]
    ],
  ...
  "vibe-developer-pipeline": {
    lead: "This page traces a product idea from first discussion to a fully ready system. KVDF-led delivery is local-first, but planning now has a mandatory file-first State Resync step before the next Evolution is recommended. For app work, current app files, docs, specs, source, and tests outrank remote-provider state; this is a file-first source-of-truth rule. Git is historical evidence; GitHub is optional secondary provider/plugin. The pipeline still starts with file-based questions, complete answers, current-state reports, workspace boundary checks, truth reconciliation, system design docs, delivery mode, scorecards, Roadmap Train / Evo Sprint queue state, Evolution planning, task generation, implementation, verification, local archive, and handoff. For KVDF Core solo-owner work, the default GitHub path is direct-to-main. Branch and PR handoff stays optional for team, protected-repo, or risky experimental work.",
    beginner: "Use this when you need the full path from idea to a system-ready app. It shows how the vibe developer gathers answers, confirms the target repo, performs file-first State Resync before planning, reads the current-state and truth reports, produces the planning pack, chooses local-only handoff or the default solo-owner direct-to-main path for KVDF Core, and uses branch/PR only when the repo or risk profile requires it.",
    sections: [
      ...
      ["State Resync before planning", "Confirm the current repo state, latest main, merged history, release tags, roadmap docs, and app manifests before the next Evolution is recommended. Treat local runtime state as supporting evidence, not the highest authority, and keep file-first evidence ahead of remote provider history. This is the file-first source-of-truth rule."],
      ["Current-State Report", "Rebuild the live repo/workspace summary first so the next recommendation can be based on the current branch, current workspace, allowed paths, forbidden paths, and stale candidates instead of older snapshots."],
      ["Truth Reconciliation", "Compare source code, docs, tests, runtime state, and generated reports before declaring something implemented. Runtime-only or docs-only claims should not outrank source evidence."],
      ["Roadmap Train and Evo Sprint Queue", "Persist ordered version, stage, sprint, evolution, and task queues so the next session can resume FIFO state without rebuilding the plan from chat or generated reports."],
    ...
    featuredPages: [
      ["new-project", "Start a New Application", "The guided path for starting a new app workspace."],
      ["questionnaire-engine", "Questionnaire Engine", "The intake engine that collects answers and turns them into planning output."],
      ["data-design", "Data Design", "The data design guide that turns product shape into a real model."],
      ["delivery-mode", "Choose Agile or Structured", "How to choose the delivery style that fits the project."],
      ["task-governance", "Task Governance", "How approved work becomes tracked implementation."],
      ["app-boundary", "App Boundary Governance", "How workspace boundaries keep app work safe and separate."],
      ["dashboard-monitoring", "Live Dashboard", "How the workspace shows readiness, blockers, and next action."]
    ],
    steps: [
      "Capture the idea",
    ...
    details: [
      ...
      ["System design docs", [
        "The planning pack should record the product scope, architecture direction, data design, and UI/UX assumptions in a form that another reviewer can inspect.",
        "These documents are the bridge between the idea and the tasks. If they are weak, the tasks will be weak too."
      ]],
      ["Scorecards and Evolution", [
        "Scorecards help decide whether the plan is strong enough to move forward or whether it needs more work first.",
        "Once the scorecard is locked, the result can become Evolution work for the framework or remain a normal app task track."
      ]],
      ["Tasks to implementation", [
        "Approved tasks should be small enough to finish, verify, and explain. The pipeline should not turn one vague idea into a giant unsafe bundle.",
        "The task list is only valid when it clearly traces back to the approved planning pack and the locked scorecard state."
      ]],
      ["Release readiness", [
        "A pipeline is not finished when the code compiles. It is finished when the workspace can prove that the plan, the implementation, and the evidence all match.",
        "That is why the final stage includes validation, handoff, and a clear release signal."
      ]]
    ],
    checklist: [
...
Object.assign(docs.ar.pages, {
  "ai-tool": {
    lead: "أداة الذكاء الاصطناعي هي عقدة المساعد. يمكنها التفكير والشرح والاقتراح والتنفيذ فقط داخل تاسك معتمد. لا تعتمد الخطة بنفسها ولا توسع النطاق بصمت.",
    beginner: "استخدم هذه الصفحة عندما تعمل مع Codex أو أي أداة AI داخل كبيري. القاعدة بسيطة: خطط أولاً، قسم العمل إلى تاسكات، انتظر الموافقة، ثم نفذ تاسك واحداً في كل مرة وتوقف.",
    sections: [
      ["القاعدة المشتركة", "لا تعدل الملفات حتى يتم تقسيم الـ Evo أو المهمة إلى خطوات واضحة مع التبعيات ومعايير القبول."],
      ["ما الذي يجوز لأداة AI فعله", "قراءة الخطة المعتمدة، المساعدة في تحسين تقسيم المهام، تنفيذ التاسك الحالي، تلخيص الدليل، ثم التوقف عند اكتمال التاسك."],
      ["ما الذي لا يجوز", "لا يجوز دمج تغييرات غير مرتبطة، أو تجاوز الموافقة، أو اختراع نطاق مفقود، أو الاستمرار بعد التاسك الحالي بدون سؤال."],
      ["أفضل ممارسة", "تعامل مع كل تاسك كسلسلة محكومة صغيرة. إذا بدا التغيير أكبر من شريحة واحدة، ارجع إلى التخطيط قبل التعديل."]
    ],
    steps: ["اقرأ الخطة", "أكد النطاق", "نفذ تاسك واحد", "تحقق من الناتج", "توقف وانتظر"],
    details: [
      ["قواعد التوقف", ["إذا تغير النطاق، توقف فوراً واطلب موافقة.", "إذا لم تكن الخطوة التالية واضحة، لا تخمن."]],
      ["تعريف النجاح", ["تنجح أداة AI عندما تلتزم بتاسك معتمد واحد، وتترك أثراً واضحاً، ولا تحول التخطيط إلى تنفيذ مخفي."]]
    ],
    checklist: ["تم تقسيم التخطيط إلى تاسكات.", "معايير القبول مكتوبة.", "الموافقة موجودة قبل التعديل.", "تاسك واحد فقط نشط.", "تم التحقق قبل الاستمرار."],
    commands: ["knowledge/governance/KVDF_WORKFLOW_INSTRUCTIONS.md", "docs/reports/KVDF_WORKSPACE_BOUNDARY_IMPLEMENTATION_CHECKLIST.md"]
  },
  "ai-tool-hub": {
    lead: "هذه هي بوابة أداة AI. توجه المساعد إلى عقدة العمل الصارمة وقاعدة التاسك المعتمد قبل بدء أي تنفيذ.",
    beginner: "ابدأ من هنا إذا كنت تستخدم Codex أو أي أداة AI. هذه البوابة توضح أين توجد تعليمات أداة AI وكيف تنتقل من التخطيط إلى تنفيذ تاسك واحد فقط بدون تخمين.",
    sections: [
      ["الدليل الأساسي", "صفحة أداة AI تشرح كيف يجب أن يتصرف المساعد: خطط أولاً، وتوقف بعد كل تاسك، ولا توسع النطاق بصمت."],
      ["استخدمها عندما", "تحتاج إلى بوابة سريعة إلى تعليمات أداة AI، أو سياسة العمل المشتركة، أو قواعد التوقف الخاصة بالدور."],
      ["صفحات مرتبطة", "من هنا يمكنك الانتقال إلى بوابة مطور كبيري أو بوابة مطور Vibe إذا تغيّر نوع العمل."]
    ],
    featuredPages: [
      ["ai-tool", "تعليمات أداة AI", "عقد التشغيل الصارم لـ Codex وأي أدوات AI أخرى."],
      ["developer-onboarding", "تأهيل المطور", "مسار التأهيل القياسي لمسار عمل كبيري وبوابة التخطيط."],
      ["what-is", "نظرة عامة", "نظرة عامة على كيف يعمل كبيري ومكان أسطح الأدوار."]
    ],
    steps: ["افتح تعليمات أداة AI", "اقرأ سياسة العمل", "تحقق من شريحة التاسك", "نفذ تاسكاً معتمداً واحداً", "توقف وانتظر"],
    details: [
      ["روابط كانونيكية", ["`knowledge/governance/KVDF_WORKFLOW_INSTRUCTIONS.md`", "`docs/site/pages/ar/ai-tool.html`", "`docs/site/pages/ar/vibe-developer.html`"]]
    ],
    checklist: ["للمساعد تاسك معتمد واحد.", "حزمة التخطيط معتمدة.", "سياسة العمل متاحة.", "تغيير النطاق يسبب توقفاً."],
  ...
  "vibe-developer-hub": {
    ...
    featuredPages: [
      ...
      ["new-project", "بدء تطبيق جديد", "مسار التخطيط والموافقة الصارم لمساحات العمل الجديدة."],
      ["questionnaire-engine", "محرك الأسئلة", "طبقة الإدخال والتخطيط التي تنتج الخطة المعتمدة الجاهزة للتاسكات."],
      ["data-design", "تصميم البيانات", "إرشادات نمذجة البيانات لمساحات العمل والـ surface scopes."],
      ["task-governance", "حوكمة التاسكات", "كيف تُنشأ التاسكات المعتمدة وتُتابَع وتُنجَز."],
      ["app-boundary", "حوكمة حدود التطبيقات", "كيف تحمي حدود مساحة العمل العمل المشترك والعمل الخاص بكل سطح."],
      ["dashboard-monitoring", "الداشبورد الحي", "داشبورد مساحة العمل الذي يعرض الجاهزية والمعوقات والخطوة التالية."]
    ],
  ...
  "vibe-developer-pipeline": {
    lead: "تتبع هذه الصفحة رحلة فكرة المنتج من أول نقاش إلى نظام جاهز بالكامل. KVDF-led delivery يبدأ محليًا أولًا، لكن التخطيط الآن يمر بخطوة State Resync إلزامية ومبنية على الملفات قبل اقتراح Evolution التالية. في عمل التطبيقات، ملفات التطبيق الحالية والـ docs والـ specs والـ source والاختبارات تتقدم على الحالة البعيدة؛ هذه قاعدة file-first source-of-truth. Git دليل تاريخي؛ وGitHub optional secondary provider/plugin. ما زال المسار يبدأ بأسئلة داخل ملف، إجابات مكتملة، وثائق التصميم، نمط التسليم، scorecards، تخطيط Evolution، توليد التاسكات، التنفيذ، التحقق، الأرشفة المحلية، ثم handoff. في KVDF Core وعندما يكون المالك هو المطوّر الوحيد، المسار الافتراضي عبر GitHub هو direct-to-main. مسار branch وPR يبقى اختياريًا فقط للفرق أو المستودعات المحمية أو التجارب عالية المخاطر.",
    beginner: "استخدم هذه الصفحة عندما تحتاج المسار الكامل من الفكرة إلى تطبيق جاهز للنظام. توضح كيف يجمع مطور Vibe الإجابات، يؤكد الريبو الهدف، ينفذ State Resync المبني على الملفات قبل التخطيط، يبني حزمة التخطيط، يختار local-only handoff أو direct-to-main الافتراضي في KVDF Core، ويستخدم branch/PR فقط عندما يفرضه الريبو أو مستوى المخاطرة.",
    sections: [
      ...
      ["State Resync قبل التخطيط", "أكد حالة الريبو الحالية، وأحدث main، والدمج المضموم، ووسوم الإصدارات، ووثائق roadmap، وملفات manifest قبل اقتراح Evolution التالية. اعتبر الحالة المحلية runtime evidence داعمة وليست أعلى سلطة، وأبقِ دليل الملفات أعلى من أي history بعيدة. هذه قاعدة file-first source-of-truth."],
    ...
    featuredPages: [
      ["new-project", "بدء تطبيق جديد", "المسار الموجه لبدء مساحة عمل جديدة للتطبيق."],
      ["questionnaire-engine", "محرك الأسئلة", "محرك الاستقبال الذي يجمع الإجابات ويحوله إلى ناتج تخطيطي."],
      ["data-design", "تصميم البيانات", "دليل تصميم البيانات الذي يحول شكل المنتج إلى نموذج فعلي."],
      ["delivery-mode", "اختيار Agile أو Structured", "كيف تختار أسلوب التسليم المناسب للمشروع."],
      ["task-governance", "حوكمة التاسكات", "كيف يتحول العمل المعتمد إلى تنفيذ متابع."],
      ["app-boundary", "حوكمة حدود التطبيقات", "كيف تحافظ حدود مساحة العمل على أمان العمل وتفصله."],
      ["dashboard-monitoring", "الداشبورد الحي", "كيف تعرض مساحة العمل الجاهزية والمعوقات والخطوة التالية."]
    ],
    steps: [
      "اجمع الفكرة",
    ...
    details: [
      ...
      ["وثائق تصميم النظام", [
        "يجب أن تسجل حزمة التخطيط نطاق المنتج، والاتجاه المعماري، وتصميم البيانات، وافتراضات UI/UX بشكل يمكن لمراجع آخر أن يفحصه.",
        "هذه الوثائق هي الجسر بين الفكرة والتاسكات. إذا كانت ضعيفة فالتاسكات ستكون ضعيفة أيضاً."
      ]],
      ["الـ scorecards والتطور", [
        "تساعد الـ scorecards على تحديد هل الخطة قوية بما يكفي للمتابعة أم تحتاج إلى مزيد من العمل أولاً.",
        "بعد إقفال السكور كارد يمكن أن تتحول النتيجة إلى Evolution للفريمورك أو تبقى في مسار تاسكات التطبيق."
      ]],
      ["من التاسكات إلى التنفيذ", [
        "يجب أن تكون التاسكات المعتمدة صغيرة بما يكفي لكي تُنجز وتتحقق وتُشرح. لا يجب أن يحول المسار فكرة واحدة غامضة إلى حزمة كبيرة غير آمنة.",
        "قائمة التاسكات لا تصبح صالحة إلا عندما تتتبع بوضوح حزمة التخطيط المعتمدة وحالة السكور كارد المقفلة."
      ]],
      ["جاهزية التسليم", [
        "المسار لا ينتهي عندما ينجح البناء. ينتهي عندما تستطيع المساحة إثبات أن الخطة والتنفيذ والأدلة كلها متطابقة.",
        "ولهذا تتضمن المرحلة الأخيرة التحقق والتسليم وإشارة واضحة إلى الجاهزية للإصدار."
      ]]
    ],
    checklist: [
      "الفكرة مكتوبة بوضوح.",
...
function pageTitle(lang, slug) {
  ...
  const row = pages.find(([id]) => id === canonicalSlug);
  const page = docs[lang].pages[canonicalSlug] || {};
  if (lang === "ar") return page.arTitle || (row ? row[2] : canonicalSlug);
  return row ? row[1] : canonicalSlug;
}

function buildSearchIndex(lang) {
  return pages.map(([slug]) => {
    if (pageAliases[slug]) return null;
...
function renderNav(searchQuery = "") {
  ...
  const scope = roleNavScopes[slug] || null;
  const results = buildSearchIndex(lang)
  ...
  const summary = query
    ...
    : "";
  const scopeBanner = scope && !query
    ? `<div class="search-summary">${scope.label[lang]}</div>`
    : "";
  const empty = query && !results.length
    ...
    : "";
  nav.innerHTML = scopeBanner + summary + empty + results.map(({ slug: id, title }) => {
...
function redirectAliasIfNeeded() {
  const slug = currentSlug();
  const aliasTarget = pageAliases[slug];
  if (!aliasTarget) return false;
  window.location.replace(pagePath(currentLang(), aliasTarget));
  return true;
}
...
function renderContent() {
  ...
  const page = dict.pages[canonicalSlug] || dict.pages["what-is"];
  const title = lang === "ar" ? (page.arTitle || Object.fromEntries(pages.map(([id, , ar]) => [id, ar]))[canonicalSlug]) : Object.fromEntries(pages.map(([id, en]) => [id, en]))[canonicalSlug];
  ...
  const featuredPages = (page.featuredPages || []).length ? `
    <section class="deep-section wide-section">
      <h2>${dict.ui.roleGuides}</h2>
      <div class="section-grid">
        ${(page.featuredPages || []).map(([slugId, titleText, bodyText]) => `
          <article class="info-card">
            <h3><a href="${pagePath(lang, slugId)}">${inlineCode(titleText)}</a></h3>
            <p>${inlineCode(bodyText)}</p>
          </article>
        `).join("")}
      </div>
    </section>
...
applyDeepGuides();
if (redirectAliasIfNeeded()) {
  // Redirecting to the canonical page keeps hub URLs from behaving as separate pages.
} else {
renderNav();
renderLanguageLinks();
renderContent();
