...
status counts below are not the current implementation status and must not be
used as runtime truth. Many items that were counted as `missing` or `partial`
have since been implemented in runtime. Use `CURRENT_REPOSITORY_ANALYSIS.md`,
`GAP_REPORT.md`, and `MISSING_REQUIREMENTS_BACKLOG.md` for active status.
