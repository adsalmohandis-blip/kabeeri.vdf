...
| Multi-AI Governance | Controls Evolution-led session leadership, agent hub entries, leader leases, per-AI queues, sync/distribution of the active Evolution temporary queue, semantic merge bundles with surface planning, optional wifi packet workflow, locks, tokens, audit, and Owner verification. The governance doc and live state file are the source of truth for who leads and which queue is active, while the optional wifi integration stays transport-only and does not auto-apply packets back into governance state. | `knowledge/governance/`, `knowledge/governance/MULTI_AI_GOVERNANCE.md`, `.kabeeri/multi_ai_governance.json`, `plugins/multi_ai_governance/governance/WIFI_GOVERNANCE_PACKET_POLICY.md` |
