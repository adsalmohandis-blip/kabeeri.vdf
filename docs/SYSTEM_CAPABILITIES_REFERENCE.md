...
| Vibe App Developer Track | Build real user applications with Kabeeri as the engine. | `resume -> vibe/ask -> blueprint/questionnaire -> evolution -> temp -> tasks/capture -> branch/review/merge(optional GitHub) -> validate -> handoff` | Vibe-first intake, milestone-based app evolution, app task execution, capture records, app blueprints, prompt packs, optional GitHub sync, task trash/archive, and app-facing task governance. |
