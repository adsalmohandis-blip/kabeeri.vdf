...
| Planner Layer | Reads repo and runtime context, then recommends the next governed KVDF Core Evolution, emits a Task Punch, and prepares a Codex-ready prompt without treating branch/PR as the default path. It stays deterministic in the MVP and does not write runtime state under `.kabeeri/`. | `knowledge/governance/KVDF_PLANNER_LAYER.md`, `docs/workflows/EVOLUTION_PLANNER_WORKFLOW.md`, `packs/planner/`, `schemas/planner/`, `kvdf planner next`, `kvdf planner prompt`, `kvdf planner evolution`, `kvdf planner task-punch` |
