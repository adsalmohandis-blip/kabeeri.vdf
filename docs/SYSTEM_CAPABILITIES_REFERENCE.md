...
| AI Learning Memory | Records repeated AI mistakes, blockers, learned warnings, and fast-path solutions so future prompts can reuse the shortest safe route instead of replaying the same failures. The shared memory is track-aware for owner, vibe, and plugin work, supports combined `--track all` prompt context, and auto-synces into planner prompt generation, resume guidance, and prompt-pack composition. | `knowledge/ai_learning/AI_LEARNING_MEMORY.md`, `.kabeeri/ai_learning/failure_patterns.json`, `kvdf learn` |
...
| Security Auditor Plugin | Optional removable plugin that scans tasks, evolutions, handoff packages, and workspace changes for common secret, injection, auth, and backdoor patterns before release or handoff. It complements the core policy gate without requiring external security tools. | `plugins/security-auditor/`, `.kabeeri/security/security_auditor_scans.json`, `kvdf security-auditor`, `kvdf plugins install security-auditor`, `kvdf plugins uninstall security-auditor` |
