...
| UI Dashboard Kits | Optional removable UI dashboard kits plugin that owns the active UI checker, dashboard examples, starter templates, and snippets while keeping the legacy core execution-kit folder as a compatibility reference only. It preserves `npm run ui:check` compatibility, stays offline, and does not require Bootstrap or Tailwind packages in KVDF Core. | `plugins/ui_dashboard_kits/`, `kvdf ui-dashboard-kits status`, `kvdf ui-dashboard-kits check`, `kvdf ui-dashboard-kits examples`, `kvdf ui-dashboard-kits templates`, `kvdf ui-dashboard-kits snippets`, `kvdf plugins install ui_dashboard_kits`, `kvdf plugins uninstall ui_dashboard_kits`, `npm run ui:check` |
