...
| Planner Documentation System | Defines the foldered app-doc catalog, docs status model, and planner documentation lifecycle so Viber app planning can plan, materialize, stage, review, and approve docs before execution. | `docs/workflows/PLANNER_DOCUMENTATION_SYSTEM.md`, `docs/workflows/VIBER_DOCUMENTATION_PIPELINE.md`, `kvdf planner docs catalog`, `kvdf planner docs plan`, `kvdf planner docs materialize`, `kvdf planner docs status`, `kvdf planner docs apply-stage`, `kvdf planner docs review` |
...
| Planner Visual Renderer Plugin | Optional presentation plugin that turns planner visual JSON into Mermaid text, a planning board, a scope map, and a markdown visual report. The Core Planner stays the source of truth; the plugin only renders the existing visual model and can be installed or removed without changing planner logic. The native planner visual and pipeline commands keep stdout-only by default, launch a browser preview only with `--open`, suppress browser launch with `--no-open`, and request fullscreen with `--fullscreen`. | `plugins/planner_visual/`, `kvdf planner-visual status`, `kvdf planner-visual render`, `kvdf planner-visual export`, `kvdf planner visual`, `kvdf planner pipeline`, `kvdf plugins install planner-visual`, `kvdf plugins uninstall planner-visual` |
