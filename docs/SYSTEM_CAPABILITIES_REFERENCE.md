...
| Bootstrap UI Asset Provider | Optional removable UI asset plugin that keeps Bootstrap out of the KVDF Core dependency graph while still making copied Bootstrap CSS and JavaScript available for explicitly opted-in dashboard or docs surfaces. The plugin is manifest-driven, disabled by default, and safe to remove without affecting Core startup or rendering. | `plugins/bootstrap_ui/`, `kvdf bootstrap-ui status`, `kvdf bootstrap-ui assets`, `kvdf bootstrap-ui snippet`, `kvdf plugins install bootstrap_ui`, `kvdf plugins uninstall bootstrap_ui` |
