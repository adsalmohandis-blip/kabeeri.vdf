...
| --- | --- | --- | --- |
| Framework Owner Track | Maintain and extend Kabeeri itself. | `resume -> evolution priorities -> evolution temp -> validate -> verify -> commit on main -> push origin main` | Evolution Steward, framework governance, release readiness, dependency mapping, and follow-up tasks for Kabeeri internals. |
| Vibe App Developer Track | Build real user applications with Kabeeri as the engine. | `resume -> vibe/ask -> blueprint/questionnaire -> evolution -> temp -> tasks/capture -> handoff (local-first) -> branch/PR optional for team/protected/risky work -> validate` | Vibe-first intake, milestone-based app evolution, app task execution, capture records, app blueprints, prompt packs, local-first handoff, optional GitHub sync, task trash/archive, and app-facing task governance. |
...
- System development docs site: `plugins/kvdf_dev/docs/site/index.html`
...
| Workflow Instructions | Defines the shared planning gate, task slicing, approval rules, stop rules, and role behavior for AI Tool and vibe developer work. Vibe evolution is local-first by default, milestone-based, and can optionally mirror branches and PRs to GitHub while the local workspace remains the source of truth. KVDF Core solo-owner work now defaults to direct-to-main instead of branch/PR. | `knowledge/governance/KVDF_WORKFLOW_INSTRUCTIONS.md`, `docs/site/pages/en/ai-tool.html`, `docs/site/pages/en/vibe-developer.html` |
...
| Core Plugin Extraction Audit | Produces a read-only extraction audit that classifies what should stay in KVDF Core and what should move into optional plugins without modifying files or requiring `.kabeeri` runtime state. | `docs/workflows/CORE_PLUGIN_EXTRACTION_PROGRAM.md`, `docs/reports/CORE_PLUGIN_EXTRACTION_AUDIT.md`, `kvdf plugin-extraction audit` |
| Naming Governance Core | Generates deterministic, track-separated machine IDs and human-readable titles for plans, versions, evolutions, and tasks; scans runtime state for invalid, missing, duplicate, and cross-track naming issues; and supports dry-run migration guidance without writing runtime state. | `docs/workflows/NAMING_GOVERNANCE_STANDARD.md`, `kvdf naming preview`, `kvdf naming validate`, `kvdf naming migrate --dry-run` |
...
| Task Lifecycle Engine | Tracks tasks through intake, ready, execution, validation, closure, blocked, and archived stages with visible next-action guidance, trash-aware status lookups, and full-task coverage reporting. Viber/App tasks stop at closure by default after `done` / `completed` / `closed`, and explicit archive confirmation is required before they move into trash; the lifecycle and trash outputs surface the retention warning and the default 30-day recovery window. | `.kabeeri/tasks.json`, `.kabeeri/task_trash.json`, `kvdf task lifecycle`, `kvdf task status`, `kvdf task coverage` |
...
| Task Tracking And Governance | Manages approved work from intake to verification, keeps task governance policy, exposes a visible lifecycle board from intake to ready, execution, validation, closure, and archived states, stores durable execution fields and task memory for resumable sessions, breaks active work into full-task-coverage queues, archives completed tasks into a 30-day trash bucket with restore and sweep support, and exposes a focused live task tracker JSON for dashboards and VS Code-style surfaces. For Viber/App tasks, closure is the default terminal state and archive/trash requires explicit confirmation; the trash output includes the recoverable-until-expiration warning. Owner and app tracks are filtered separately so each workspace dashboard only sees its own task analysis unless a linked summary is explicitly requested. | `knowledge/task_tracking/`, `knowledge/task_tracking/TASK_GOVERNANCE.md`, `.kabeeri/dashboard/task_tracker_state.json`, `.kabeeri/task_trash.json`, `kvdf task`, `kvdf task lifecycle`, `kvdf task coverage`, `kvdf task memory`, `kvdf task trash` |
...
| App Plugin Suite | Provides installable app-track plugin bundles for company profiles, news websites, blogs, ecommerce mobile apps, CRM, and POS with strict runtime pipelines, plugin-state gating, bundle-contract status, and next-exact-action reporting. Each plugin family should keep its manifest, docs, and runtime shape aligned so operators can swap bundles without relearning the workflow. | `plugins/company_profile/`, `plugins/news_website/`, `plugins/blog/`, `plugins/ecommerce_mobile_app/`, `plugins/crm/`, `plugins/pos/`, `kvdf company-profile`, `kvdf news-website`, `kvdf blog`, `kvdf ecommerce-mobile-app`, `kvdf crm`, `kvdf pos` |
| Ecommerce Builder | Builds ecommerce systems through a strict runtime pipeline with installable/uninstallable plugin-state gating, a durable commerce workspace record, questionnaire intake, brief/design/module/task generation, approval batching, bundle-contract reporting, and release handoff. The business-type pack mirrors the booking pack structure so shared archetype guidance stays in parity across store, marketplace, digital product, subscription, and services modes. | `plugins/ecommerce_builder/`, `.kabeeri/ecommerce.json`, `kvdf ecommerce`, `kvdf plugins install ecommerce-builder`, `kvdf plugins uninstall ecommerce-builder` |
| Booking Builder | Builds booking and reservation products through a strict runtime pipeline with installable/uninstallable plugin-state gating, a durable booking workspace record, questionnaire intake, brief/design/module/task generation, approval batching, bundle-contract reporting, and release handoff. The business-type pack mirrors the ecommerce pack structure so shared archetype guidance stays in parity across appointments, services, classes, hotels, and events modes. | `plugins/booking_builder/`, `.kabeeri/booking.json`, `kvdf booking`, `kvdf plugins install booking-builder`, `kvdf plugins uninstall booking-builder` |
...
| App Boundary Governance | Allows same-product multi-app workspaces while blocking unrelated products in one folder, and keeps the portable app-doc package inside the app root. | `knowledge/governance/APP_BOUNDARY_GOVERNANCE.md`, `knowledge/governance/APP_DOCS_STANDARD.md`, `knowledge/governance/APP_DOCS_STANDARD.json`, `kvdf app` |
| Portable App Docs Package | Stores the canonical app-folder docs package for product, UX, system design, data design, technical diagrams, page maps, runtime, operations, governance, and enterprise notes so the app can move to another system without losing product knowledge. The standard keeps `00-executive-summary.md` first, leaves discovery and master navigation unnumbered, and requires a contiguous numbered sequence after `00`. | `knowledge/governance/APP_DOCS_STANDARD.md`, `knowledge/governance/APP_DOCS_STANDARD.json`, `workspaces/apps/<app-slug>/docs/`, `workspaces/apps/<app-slug>/assets/`, `kvdf app workspace create`, `kvdf app workspace validate` |
...
| Multi-AI Governance | Controls Evolution-led session leadership, agent hub entries, leader leases, per-AI queues, sync/distribution of the active Evolution temporary queue, the Evolution assignment bridge for master/developer task distribution, the generated two-laptop workflow handoff report, semantic merge bundles with surface planning, IDE window governance for shared workspaces, local project governance for the same machine and repository across IDEs and terminals, Wi-Fi/LAN governance through `wifi_data_sharing`, Kabeeri / KVDOS Cloud governance through the `kcloud_data_sharing` transport boundary when available, GitHub provider governance through `github_provider` when GitHub work must be approved before execution, locks, tokens, audit, and Owner verification. The governance doc and live state file are the source of truth for who leads and which queue is active, while the IDE layer records local presence, leases, conflicts, and policy decisions, the local-project layer records machine/project/client/session/lease/policy state across the whole repo, the Wi-Fi/LAN layer records node identity, pairing, permissions, tokens, leases, conflicts, and policy decisions while the transport plugin stays transport-only and does not auto-apply packets back into governance state, the cloud layer records tenant/project/node/role mapping, permissions, tokens, leases, packet checks, approvals, conflicts, evidence, and audit locally while a future cloud transport layer only carries packets, and the GitHub provider layer records provider connections, repo mapping, governed operation requests, issue/PR/check orchestration, approvals, evidence, and audit locally while `github_provider` performs the actual provider work after approval. Phase 1 governs IDE-window identity, tool presence, leases, conflicts, and owner-approved high-risk actions, Phase 2 adds local project governance, Phase 3 adds Wi-Fi/LAN governance and explicit packet decision recording without changing authority, Phase 4 adds KCloud governance without moving authority into `kcloud_data_sharing`, and Phase 5 adds GitHub provider governance without moving authority into `github_provider` or GitHub itself. | `knowledge/governance/`, `knowledge/governance/MULTI_AI_GOVERNANCE.md`, `.kabeeri/multi_ai_governance.json`, `plugins/multi_ai_governance/docs/CASE_1_IDE_WINDOW_GOVERNANCE.md`, `plugins/multi_ai_governance/docs/CASE_2_LOCAL_PROJECT_GOVERNANCE.md`, `plugins/multi_ai_governance/docs/CASE_3_WIFI_LAN_GOVERNANCE.md`, `plugins/multi_ai_governance/docs/CASE_4_KCLOUD_GOVERNANCE.md`, `plugins/multi_ai_governance/docs/CASE_5_GITHUB_PROVIDER_GOVERNANCE.md`, `plugins/multi_ai_governance/governance/WIFI_GOVERNANCE_PACKET_POLICY.md` |
...
| KVDF Dev System / Framework Owner Track / Evolution Steward | Acts as the single framework-development backlog by recording requested framework changes as milestone-style evolutions, keeping ordered priorities, a canonical KVDF restructure roadmap, temporary execution queues for the active priority, resumable execution reports, scorecards for review and comparison, an optional scorecard-to-Evolution materialization path when explicitly requested, a governed batch-exe queue for ready and approved tasks, auto-assigning missing EVO assignees to the active Multi-AI leader or codex fallback, plus the plugin-owned `kvdf task packet` surface for control-plane packet compilation, `kvdf task executor-contract` for packet-only AI boundary rules, and `kvdf task batch-run` for starting approved tasks in governed priority order or previewing packet/contract modes. Each evolution now carries local-first branch, review, merge-target, optional GitHub sync, and task trash/archive metadata so the local workspace remains the source of truth even when GitHub sync is enabled. KVDF Core solo-owner delivery defaults to direct-to-main. This bundle is packaged as a removable plugin so it stays separated from app-building plugins and app-track workflows. | `knowledge/governance/EVOLUTION_STEWARD.md`, `.kabeeri/evolution.json`, `kvdf evolution`, `kvdf evolution roadmap`, `kvdf evolution scorecards`, `kvdf evolution report`, `kvdf evolution batch-exe`, `kvdf batch-exe`, `kvdf task packet`, `kvdf task executor-contract`, `kvdf task batch-run`, `kvdf plugins install kvdf-dev`, `kvdf plugins uninstall kvdf-dev` |
| State Resync And Planner Drift Guard | Rebuilds a file-first Current-State Report, persists the current-state snapshot into `.kabeeri/state_resync/`, compares current files against roadmap/runtime claims, and blocks planner authority until the report is fresh. The guard keeps Git as historical evidence, GitHub as optional secondary provider/plugin evidence, and chat as supporting context only. The policy also treats KVDOS and app workspaces as separate from KVDF Core so stale roadmap/report files cannot override the live repo identity or workspace boundary. | `knowledge/governance/KVDF_WORKFLOW_INSTRUCTIONS.md`, `docs/workflows/KVDF_STATE_RESYNC.md`, `docs/workflows/CURRENT_STATE_REPORT_TEMPLATE.md`, `packs/planner/state-resync.prompt.md`, `schemas/runtime/current-state-report.schema.json`, `schemas/runtime/state-resync-state.schema.json`, `schemas/runtime/source-truth-policy.schema.json`, `kvdf state resync`, `kvdf planner guard`, `kvdf planner current-state`, `kvdf planner boundary`, `kvdf planner stale-state` |
| Planner Layer | Shared native planning capability that reads repo and runtime context, then rebuilds the current-state report, checks workspace boundaries, reconciles source/docs/runtime truth, recommends the next governed Evolution, chooses a planning method, emits a Task Punch, persists proposed plans under `.kabeeri/planner.json`, reviews and resumes planner state, classifies stale roadmap/report/runtime files, plans and materializes foldered draft documentation files, evaluates Viber version-to-publish readiness, docs/design gate state, version/evolution/task-punch gate state, stage-transition state, and final execution gate state, builds shared Roadmap Train and Evo Sprint Queue state for owner and Viber tracks, and exposes a reusable `roadmap_train_summary` plus `evo_sprint_queue` snapshot for dashboard, prompt, visual, and pipeline reuse. It also prepares a Codex-ready prompt from approved runtime state, materializes approved plans into Evolution and Task Punch runtime records, emits an execution-readiness visual planning model, produces idea-to-evolution pipeline reports, and now syncs planner/pipeline summaries into dashboard/live state for owner, vibe/app, or plugin tracks while honoring State Resync freshness before authoritative next-evolution recommendations. Planner also carries a `git_context` boundary report so Viber/app prompts and pipeline outputs can block branch/push/PR suggestions when the app workspace resolves Git to KVDF Core. When the optional `ui_ux_intelligence` plugin is available, Planner can also include offline recommendation, checklist, docs, review, visual, prompt, handoff-pack, pattern-library, implementation-guidance, prompt-pack, and evidence/readiness summaries via explicit include flags without making the provider mandatory. When the optional `tailwind_ui` plugin is requested, Planner can surface read-only Tailwind provider, planner-guidance, docs-guidance, review, visual, and prompt summaries without making Tailwind a Core dependency. The planner state records proposed, approved, rejected, completed, materialized, and version-ready plans so the approval gate is durable and the current plan can be reused by prompt, visual, materialize, pipeline, review, resume, docs, version, train, truth, and dashboard commands. KVDF Core stays direct-to-main by default, vibe/app planning stays local-first by default, plugin planning protects plugin mount/runtime state, KVDF does not auto-publish, and the planner carries an explicit source-control provider model instead of assuming GitHub branch/PR as mandatory. | `knowledge/governance/KVDF_PLANNER_LAYER.md`, `docs/reports/KVDF_PLANNER_LAYER.md`, `docs/workflows/EVOLUTION_PLANNER_WORKFLOW.md`, `docs/workflows/SOURCE_CONTROL_PROVIDER_MODEL.md`, `docs/workflows/ROADMAP_TRAIN_AND_EVO_SPRINT_QUEUE.md`, `docs/workflows/OWNER_ROADMAP_PIPELINE.md`, `docs/workflows/VIBER_RELEASE_TRAIN.md`, `docs/workflows/IDEA_TO_EVOLUTION_PIPELINE.md`, `docs/workflows/PLANNER_SELF_PLANNING_ENGINE.md`, `docs/workflows/PLANNER_DOCUMENTATION_SYSTEM.md`, `docs/workflows/VIBER_DOCUMENTATION_PIPELINE.md`, `docs/workflows/VIBER_VERSION_TO_PUBLISH_CONTROL.md`, `docs/workflows/SOURCE_OF_TRUTH_AND_WORKSPACE_BOUNDARY_POLICY.md`, `packs/planner/`, `schemas/planner/`, `kvdf planner current-state`, `kvdf planner boundary`, `kvdf planner stale-state`, `kvdf planner next`, `kvdf planner method`, `kvdf planner auto`, `kvdf planner review`, `kvdf planner resume`, `kvdf planner docs`, `kvdf planner version`, `kvdf planner docs catalog`, `kvdf planner docs plan`, `kvdf planner docs materialize`, `kvdf planner docs status`, `kvdf planner docs apply-stage`, `kvdf planner docs review`, `kvdf planner propose`, `kvdf planner approve`, `kvdf planner current`, `kvdf planner reject`, `kvdf planner complete`, `kvdf planner materialize`, `kvdf planner prompt`, `kvdf planner visual`, `kvdf planner pipeline`, `kvdf planner evolution`, `kvdf planner task-punch`, `kvdf planner train build`, `kvdf planner train status`, `kvdf planner train next`, `kvdf planner train advance`, `kvdf planner train visual`, `kvdf planner train readiness`, `kvdf truth audit`, `kvdf truth feature`, `kvdf planner truth`, `kvdf evolution reconcile` |
| Evolution Ordering Gate | Validates generic Viber/App evolution categories before task generation. The gate enforces boundary-first, discovery-before-tasking, safety-before-execution, execution-before-packaging, and approval rules so task punches cannot be generated from draft or misordered slices. | `schemas/runtime/evolution-order-policy.schema.json`, `schemas/runtime/evolution-order-validation.schema.json`, `docs/workflows/VIBER_EVOLUTION_ORDERING_POLICY.md`, `kvdf evolution validate-order`, `kvdf questionnaire generate-tasks` |
| Viber Pipeline Stage Order | Exposes the ordered Viber app lifecycle from idea through questionnaire, brief, state resync, docs/design gates, versioning, evolutions, task punches, approval, materialization, execution, validation, handoff, dashboard update, learning capture, and closeout. It also exposes stage-transition state so the next stage cannot be skipped. | `kvdf planner pipeline --track vibe`, `schemas/planner/viber-pipeline-state.schema.json`, `docs/workflows/VIBER_APP_DELIVERY_PIPELINE.md` |
| Viber Pipeline Stage Inspector | Reads a single Viber pipeline stage or the full ordered list in read-only mode and reports its order, group, required gates, evidence, blockers, warnings, and next safe action without generating tasks or executing work. | `kvdf planner pipeline inspect --track vibe --stage <stage>`, `kvdf planner pipeline inspect --track vibe --all`, `schemas/planner/viber-pipeline-stage-inspection.schema.json`, `docs/workflows/VIBER_APP_DELIVERY_PIPELINE.md` |
| Canonical Truth And Reconciliation | Compares source, docs, tests, runtime state, current-state reports, and generated snapshots so KVDF does not treat runtime or generated files as implementation proof. The truth layer marks implemented, documented-only, runtime-only, stale-planned, stale-recommended, and ambiguous items, then tells Planner and Evolution when to stop and reconcile instead of trusting stale backlog records. | `docs/workflows/CANONICAL_TRUTH_AND_RECONCILIATION.md`, `kvdf truth audit`, `kvdf truth feature`, `kvdf planner truth`, `kvdf evolution reconcile`, `kvdf planner current-state`, `kvdf planner boundary`, `kvdf planner stale-state` |
| Planner Documentation System | Defines the foldered app-doc catalog, docs status model, portable docs mapping, and planner documentation lifecycle so Viber app planning can plan, materialize, stage, review, and approve docs before execution without replacing the canonical portable app-doc package. | `docs/workflows/PLANNER_DOCUMENTATION_SYSTEM.md`, `docs/workflows/VIBER_DOCUMENTATION_PIPELINE.md`, `docs/workflows/VIBER_APP_DELIVERY_PIPELINE.md`, `kvdf planner docs catalog`, `kvdf planner docs plan`, `kvdf planner docs materialize`, `kvdf planner docs status`, `kvdf planner docs apply-stage`, `kvdf planner docs review` |
| Source Control Provider Model | Explains no-source-control, local-only, direct-to-main, branch, and branch+PR modes so Git is a provider, GitHub stays optional, and future providers remain replaceable. It also includes the Viber/app Git boundary guard and the `kvdf source-control context` classification report so app work cannot accidentally branch or push against KVDF Core. | `docs/workflows/SOURCE_CONTROL_PROVIDER_MODEL.md`, `knowledge/governance/KVDF_PLANNER_LAYER.md`, `docs/cli/CLI_COMMAND_REFERENCE.md`, `kvdf source-control context`, `kvdf planner next`, `kvdf planner propose` |
| Planner Visual Renderer Plugin | Optional presentation plugin that turns planner visual JSON into Mermaid text, a planning board, a scope map, an execution-readiness report, and a markdown visual report. The Core Planner stays the source of truth; the plugin only renders the existing visual model and can be installed or removed without changing planner logic. The native planner visual and pipeline commands keep stdout-only by default, launch a browser preview only with `--open`, suppress browser launch with `--no-open`, and request fullscreen with `--fullscreen`. KVDF does not auto-publish. | `plugins/planner_visual/`, `kvdf planner-visual status`, `kvdf planner-visual render`, `kvdf planner-visual export`, `kvdf planner visual`, `kvdf planner pipeline`, `kvdf plugins install planner-visual`, `kvdf plugins uninstall planner-visual` |
...
| Capability Partition Matrix | Classifies the complete KVDF capability catalog into `kabeeri-core`, `plugins/kvdf_dev`, and `workspaces/apps/<app-slug>` ownership buckets, then publishes the authoritative split and boundary rules. | `docs/reports/KVDF_CORE_PLUGIN_CAPABILITY_SPLIT_STUDY.md`, `kvdf evolution partition` |
...
| UI UX Intelligence Plugin | Optional standalone UI/UX intelligence plugin that works offline in the MVP, provides deterministic product detection, a local catalog/search layer over the relocated CSV data, style and palette recommendations, typography and layout guidance, chart and icon guidance, accessibility checklists, UI/UX checklist generation, Markdown-ready docs sections, lightweight audit reporting, anti-pattern detection, Viber docs support, UI readiness scorecards, stage gates, readiness checks, handoff packs, design tokens, component blueprints, screen blueprints, UI pattern libraries, implementation guidance, Codex-ready prompt packs, evidence manifests, visual QA contracts, acceptance gates, regression checklists, knowledge-pack versioning, catalog health checks, governance registry reports, and safe upgrade planning. Planner, dashboard, and Viber pipeline commands can consume it only when explicitly included or when a safe availability summary is desired; the plugin remains removable and never becomes a hard dependency. Phase 1 used a flat `_temp_meta/` staging contract and phase 2 relocates the approved CSV data into `plugins/ui_ux_intelligence/data/` and `plugins/ui_ux_intelligence/data/stacks/`. The runtime does not read `_temp_meta/`, external GitHub, or any AI API. The checklist, docs, audit, scorecard, gate, readiness, tokens, components, screens, patterns, implementation-guidance, prompt-pack, evidence, visual-qa, acceptance-gate, regression, knowledge-pack, catalog-health, governance-registry, upgrade-plan, and governance commands do not write files unless `handoff-pack`, `prompt-pack`, or a reporting command is explicitly given an allowed `--output` path. | `plugins/ui_ux_intelligence/`, `kvdf ui-ux-intelligence status`, `kvdf ui-ux-intelligence source-status`, `kvdf ui-ux-intelligence catalog`, `kvdf ui-ux-intelligence search`, `kvdf ui-ux-intelligence recommend`, `kvdf ui-ux-intelligence design-system`, `kvdf ui-ux-intelligence checklist`, `kvdf ui-ux-intelligence docs`, `kvdf ui-ux-intelligence audit`, `kvdf ui-ux-intelligence scorecard`, `kvdf ui-ux-intelligence gate`, `kvdf ui-ux-intelligence readiness`, `kvdf ui-ux-intelligence handoff-pack`, `kvdf ui-ux-intelligence tokens`, `kvdf ui-ux-intelligence components`, `kvdf ui-ux-intelligence screens`, `kvdf ui-ux-intelligence patterns`, `kvdf ui-ux-intelligence implementation-guidance`, `kvdf ui-ux-intelligence prompt-pack`, `kvdf ui-ux-intelligence evidence`, `kvdf ui-ux-intelligence visual-qa`, `kvdf ui-ux-intelligence acceptance-gate`, `kvdf ui-ux-intelligence regression`, `kvdf ui-ux-intelligence knowledge-pack`, `kvdf ui-ux-intelligence catalog-health`, `kvdf ui-ux-intelligence governance-registry`, `kvdf ui-ux-intelligence upgrade-plan`, `kvdf ui-ux-intelligence governance`, `kvdf plugins install ui_ux_intelligence`, `kvdf plugins uninstall ui_ux_intelligence` |
| UI Dashboard Kits | Optional removable UI dashboard kits plugin that owns the active UI checker, dashboard examples, starter templates, snippets, provider metadata, and lightweight dashboard surface recommendations while keeping the legacy core execution-kit folder as a compatibility reference only. It preserves `npm run ui:check` compatibility, stays offline, and does not require Bootstrap or Tailwind packages in KVDF Core. | `plugins/ui_dashboard_kits/`, `kvdf ui-dashboard-kits status`, `kvdf ui-dashboard-kits check`, `kvdf ui-dashboard-kits examples`, `kvdf ui-dashboard-kits templates`, `kvdf ui-dashboard-kits snippets`, `kvdf ui-dashboard-kits provider`, `kvdf ui-dashboard-kits recommend`, `kvdf ui-dashboard-kits html-comment`, `kvdf plugins install ui_dashboard_kits`, `kvdf plugins uninstall ui_dashboard_kits`, `npm run ui:check` |
| Bootstrap UI Asset Provider | Optional removable UI asset plugin that keeps Bootstrap out of the KVDF Core dependency graph while still making copied Bootstrap CSS and JavaScript available for explicitly opted-in dashboard or docs surfaces. The plugin is manifest-driven, disabled by default, and safe to remove without affecting Core startup or rendering. It also exposes local verify/provider checks so surfaces can safely fall back when Bootstrap is unavailable. | `plugins/bootstrap_ui/`, `kvdf bootstrap-ui status`, `kvdf bootstrap-ui assets`, `kvdf bootstrap-ui verify`, `kvdf bootstrap-ui provider`, `kvdf bootstrap-ui snippet`, `kvdf plugins install bootstrap_ui`, `kvdf plugins uninstall bootstrap_ui` |
| AI Tool Adapter | Optional removable AI tool adapter plugin that discovers local AI tools, stores a registry in `.kabeeri/ai_tool_adapters.json`, validates governed run contracts, records policy results in `.kabeeri/ai_tool_policy_results.json`, records run evidence, exposes a provider API for capability summaries and integration checks, and publishes dashboard, readiness, evidence, and audit visibility reports while keeping `multi_ai_governance` as the authority layer. Core only routes, validates schemas, and shows read-only unavailable summaries; it never emulates tool discovery, policy gates, visibility dashboards, or execution. Legacy command aliases remain accepted for compatibility. | `plugins/ai_tool_adapters/`, `kvdf ai-tool-adapter status`, `kvdf ai-tool-adapter scan`, `kvdf ai-tool-adapter list`, `kvdf ai-tool-adapter register`, `kvdf ai-tool-adapter unregister`, `kvdf ai-tool-adapter show`, `kvdf ai-tool-adapter provider`, `kvdf ai-tool-adapter capabilities`, `kvdf ai-tool-adapter can-run`, `kvdf ai-tool-adapter evidence`, `kvdf ai-tool-adapter dashboard`, `kvdf ai-tool-adapter readiness`, `kvdf ai-tool-adapter audit`, `kvdf ai-tool-adapter test`, `kvdf ai-tool-adapter run`, `kvdf ai-tool-adapter policy`, `kvdf ai-tool-adapter policy-results`, `kvdf ai-tool-adapter policy-show`, `kvdf ai-tool-adapter runs`, `kvdf ai-tool-adapter run-show`, `kvdf ai-tool-adapter enable-execution`, `kvdf ai-tool-adapter disable-execution`, `kvdf plugins install ai_tool_adapters`, `kvdf plugins uninstall ai_tool_adapters` |
| Wi-Fi Data Sharing | Optional removable integration plugin that provides local Wi-Fi/LAN node identity, candidate discovery, owner-controlled pairing, trusted node registry, bounded package transfer, inbox quarantine, outbox retry and resumable transfer sessions, apply bridge records for approved packets, release readiness, integrity checks, backups/restores, provider visibility, readiness reporting, dashboard reporting, audit/evidence reporting, transfer security gates, quarantine policy enforcement, local simulation/stress verification, and optional transport APIs for trusted KVDF and KVDOS nodes. Core stays thin by routing only to the plugin, validating runtime state, and observing the reports; it does not perform discovery, trust promotion, inbox acceptance, outbox retry, apply-bridge mutation, backup creation, restore execution, provider negotiation, audit compilation, evidence synthesis, security policy evaluation, quarantine release, integrity scoring, release readiness, session cleanup, simulation orchestration, or data transfer itself. Phase 1 keeps network behavior disabled by default, Phase 2 adds discovery only, Phase 3 adds pairing and trust, Phase 4 adds package transfer and inbox review, Phase 5 adds a provider API plus an optional `multi_ai_governance` integration contract, Phase 6 adds transfer security gates and quarantine review, Phase 7 adds resumable transfer and outbox lifecycle without turning Core into the authority layer, Phase 8 adds dashboard, evidence, audit, and readiness reporting without adding new network behavior, Phase 9 adds local two-node simulation and stress testing without requiring real Wi-Fi, Phase 10 adds the optional `multi_ai_governance` packet workflow over wifi transport, Phase 11 makes packet consume record explicit approve/reject decisions without changing the authority model, Phase 12 adds an explicit apply bridge for already approved packets, and Phase 13 adds health, release readiness, integrity checks, and backup/restore hardening. | `plugins/wifi_data_sharing/`, `kvdf wifi-data-sharing status`, `kvdf wifi-data-sharing init`, `kvdf wifi-data-sharing state`, `kvdf wifi-data-sharing reset`, `kvdf wifi-data-sharing policy`, `kvdf wifi-data-sharing discover`, `kvdf wifi-data-sharing provider`, `kvdf wifi-data-sharing readiness`, `kvdf wifi-data-sharing dashboard`, `kvdf wifi-data-sharing audit`, `kvdf wifi-data-sharing evidence`, `kvdf wifi-data-sharing apply`, `kvdf wifi-data-sharing apply show`, `kvdf wifi-data-sharing apply reject`, `kvdf wifi-data-sharing apply cancel`, `kvdf wifi-data-sharing applied`, `kvdf wifi-data-sharing health`, `kvdf wifi-data-sharing release report`, `kvdf wifi-data-sharing backup`, `kvdf wifi-data-sharing backup create`, `kvdf wifi-data-sharing backup restore`, `kvdf wifi-data-sharing restore`, `kvdf wifi-data-sharing integrity check`, `kvdf wifi-data-sharing pairing create`, `kvdf wifi-data-sharing pairing verify`, `kvdf wifi-data-sharing trust`, `kvdf wifi-data-sharing trusted`, `kvdf wifi-data-sharing revoke`, `kvdf wifi-data-sharing server start`, `kvdf wifi-data-sharing server status`, `kvdf wifi-data-sharing package create`, `kvdf wifi-data-sharing send`, `kvdf wifi-data-sharing inbox`, `kvdf wifi-data-sharing outbox`, `kvdf wifi-data-sharing transfer sessions`, `kvdf wifi-data-sharing transfer resume`, `kvdf wifi-data-sharing transfer clean`, `kvdf wifi-data-sharing transfers`, `kvdf wifi-data-sharing security check`, `kvdf wifi-data-sharing security results`, `kvdf wifi-data-sharing quarantine`, `kvdf wifi-data-sharing quarantine show`, `kvdf wifi-data-sharing quarantine release`, `kvdf wifi-data-sharing quarantine reject`, `kvdf wifi-data-sharing advertise`, `kvdf wifi-data-sharing candidates`, `kvdf wifi-data-sharing simulate two-node`, `kvdf wifi-data-sharing simulate two-node --json`, `kvdf wifi-data-sharing simulate transfer --size 1024`, `kvdf wifi-data-sharing simulate security`, `kvdf plugins install wifi_data_sharing`, `kvdf plugins uninstall wifi_data_sharing` |
| Tailwind UI Provider | Optional removable Tailwind guidance provider plugin that keeps `tailwindcss` and `@tailwindcss/cli` out of the KVDF Core dependency graph while still exposing read-only status, verify, provider, planner-guidance, docs-guidance, html-comment, snippet, and utility-map helpers for explicit opt-in workflows. The plugin is manifest-driven, guidance-only in this phase, and safe to remove without affecting Core startup or rendering. When it is unavailable, surfaces keep the local fallback HTML/CSS path. | `plugins/tailwind_ui/`, `kvdf tailwind-ui status`, `kvdf tailwind-ui provider`, `kvdf tailwind-ui verify`, `kvdf tailwind-ui planner-guidance`, `kvdf tailwind-ui docs-guidance`, `kvdf tailwind-ui html-comment`, `kvdf tailwind-ui snippet`, `kvdf tailwind-ui utility-map`, `kvdf plugins install tailwind_ui`, `kvdf plugins uninstall tailwind_ui` |
...
feedback` stay local but summarize issue/PR/status/comment feedback records
when the workspace is in team mode.

## 21. Security Governance

Security Governance provides a lightweight local scan for common secret patterns
and a gate for high-risk findings. The gate is policy-driven and can warn or
block depending on the active security gate policy, the scope, the track, and
the lifecycle moment being evaluated.
...
kvdf security gate
kvdf security gate --task task-001
kvdf security gate --evolution current
kvdf security gate --handoff
kvdf security list
