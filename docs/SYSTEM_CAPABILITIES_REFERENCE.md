...
| AI Tool Adapters | Optional removable AI tool adapter plugin that discovers local AI tools, stores a registry in `.kabeeri/ai_tool_adapters.json`, validates governed run contracts, records run evidence, and exposes a provider API for capability summaries and integration checks while keeping `multi_ai_governance` as the authority layer. Core only routes, validates schemas, and shows read-only unavailable summaries; it never emulates tool discovery or execution. | `plugins/ai_tool_adapters/`, `kvdf ai-tool-adapters status`, `kvdf ai-tool-adapters scan`, `kvdf ai-tool-adapters list`, `kvdf ai-tool-adapters register`, `kvdf ai-tool-adapters unregister`, `kvdf ai-tool-adapters show`, `kvdf ai-tool-adapters provider`, `kvdf ai-tool-adapters capabilities`, `kvdf ai-tool-adapters can-run`, `kvdf ai-tool-adapters evidence`, `kvdf ai-tool-adapters test`, `kvdf ai-tool-adapters run`, `kvdf ai-tool-adapters runs`, `kvdf ai-tool-adapters run-show`, `kvdf ai-tool-adapters enable-execution`, `kvdf ai-tool-adapters disable-execution`, `kvdf plugins install ai_tool_adapters`, `kvdf plugins uninstall ai_tool_adapters` |
