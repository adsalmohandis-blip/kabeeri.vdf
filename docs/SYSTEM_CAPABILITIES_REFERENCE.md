...
| Planner Layer | Shared native planning capability that reads repo and runtime context, then recommends the next governed Evolution, emits a Task Punch, persists proposed plans under `.kabeeri/planner.json`, and prepares a Codex-ready prompt from approved runtime state for owner, vibe/app, or plugin tracks. KVDF Core stays direct-to-main by default, vibe/app planning stays local-first by default, plugin planning protects plugin mount/runtime state, and the planner never makes branch/PR the default path. | `knowledge/governance/KVDF_PLANNER_LAYER.md`, `docs/workflows/EVOLUTION_PLANNER_WORKFLOW.md`, `packs/planner/`, `schemas/planner/`, `kvdf planner next`, `kvdf planner propose`, `kvdf planner approve`, `kvdf planner current`, `kvdf planner reject`, `kvdf planner prompt`, `kvdf planner evolution`, `kvdf planner task-punch` |
