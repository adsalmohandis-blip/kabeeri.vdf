...
| Source Control Provider Model | Explains no-source-control, local-only, direct-to-main, branch, and branch+PR modes so Git is a provider, GitHub stays optional, and future providers remain replaceable. | `docs/workflows/SOURCE_CONTROL_PROVIDER_MODEL.md`, `knowledge/governance/KVDF_PLANNER_LAYER.md`, `docs/cli/CLI_COMMAND_REFERENCE.md`, `kvdf planner next`, `kvdf planner propose` |
