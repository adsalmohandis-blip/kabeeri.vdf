...
| Planner Visual Renderer Plugin | Optional presentation plugin that turns planner visual JSON into Mermaid text, a planning board, a scope map, and a markdown visual report. The Core Planner stays the source of truth; the plugin only renders the existing visual model and can be installed or removed without changing planner logic. The native planner visual and pipeline commands can also open a browser preview HTML file with `--open`, suppress browser launch with `--no-open`, and request fullscreen with `--fullscreen`. | `plugins/planner-visual/`, `kvdf planner-visual status`, `kvdf planner-visual render`, `kvdf planner-visual export`, `kvdf planner visual --open`, `kvdf planner pipeline --open`, `kvdf plugins install planner-visual`, `kvdf plugins uninstall planner-visual` |
...
| Security Governance | KVDF Core owns the security gate policy and the built-in scan command that blocks high-risk readiness gates. | `kvdf security`, `.kabeeri/security/` |
