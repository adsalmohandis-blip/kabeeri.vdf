...
| Planner Layer | Shared native planning capability that reads repo and runtime context, then recommends the next governed Evolution, chooses a planning method, emits a Task Punch, persists proposed plans under `.kabeeri/planner.json`, reviews and resumes planner state, materializes draft documentation files, prepares a Codex-ready prompt from approved runtime state, materializes approved plans into Evolution and Task Punch runtime records, emits a visual planning model, produces idea-to-evolution pipeline reports, and now syncs planner/pipeline summaries into dashboard/live state for owner, vibe/app, or plugin tracks. The planner state records proposed, approved, rejected, completed, and materialized plans so the approval gate is durable and the current plan can be reused by prompt, visual, materialize, pipeline, review, resume, docs, and dashboard commands. KVDF Core stays direct-to-main by default, vibe/app planning stays local-first by default, plugin planning protects plugin mount/runtime state, and the planner carries an explicit source-control provider model instead of assuming GitHub branch/PR as mandatory. | `knowledge/governance/KVDF_PLANNER_LAYER.md`, `docs/workflows/EVOLUTION_PLANNER_WORKFLOW.md`, `docs/workflows/SOURCE_CONTROL_PROVIDER_MODEL.md`, `docs/workflows/IDEA_TO_EVOLUTION_PIPELINE.md`, `docs/workflows/PLANNER_SELF_PLANNING_ENGINE.md`, `packs/planner/`, `schemas/planner/`, `kvdf planner next`, `kvdf planner method`, `kvdf planner auto`, `kvdf planner review`, `kvdf planner resume`, `kvdf planner docs`, `kvdf planner propose`, `kvdf planner approve`, `kvdf planner current`, `kvdf planner reject`, `kvdf planner complete`, `kvdf planner materialize`, `kvdf planner prompt`, `kvdf planner visual`, `kvdf planner pipeline`, `kvdf planner evolution`, `kvdf planner task-punch` |
