...
| Viber Pipeline Stage Order | Exposes the ordered Viber app lifecycle from idea through questionnaire, brief, state resync, docs, design, versioning, evolutions, task punches, approval, materialization, execution, validation, handoff, dashboard update, learning capture, and closeout. | `kvdf planner pipeline --track vibe`, `schemas/planner/viber-pipeline-state.schema.json`, `docs/workflows/VIBER_APP_DELIVERY_PIPELINE.md` |
