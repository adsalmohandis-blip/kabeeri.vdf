...
| UI UX Intelligence Plugin | Optional standalone UI/UX intelligence plugin that works offline in the MVP, provides deterministic product detection, a local catalog/search layer over the relocated CSV data, style and palette recommendations, typography and layout guidance, chart and icon guidance, accessibility checklists, UI/UX checklist generation, Markdown-ready docs sections, lightweight audit reporting, anti-pattern detection, Viber docs support, UI readiness scorecards, stage gates, readiness checks, handoff packs, design tokens, component blueprints, and screen blueprints. Planner, dashboard, and Viber pipeline commands can consume it only when explicitly included or when a safe availability summary is desired; the plugin remains removable and never becomes a hard dependency. Phase 1 used a flat `_temp_meta/` staging contract and phase 2 relocates the approved CSV data into `plugins/ui_ux_intelligence/data/` and `plugins/ui_ux_intelligence/data/stacks/`. The runtime does not read `_temp_meta/`, external GitHub, or any AI API. The checklist, docs, audit, scorecard, gate, readiness, tokens, components, screens, and handoff-pack commands do not write files unless `handoff-pack` is explicitly given an allowed `--output` path. | `plugins/ui_ux_intelligence/`, `kvdf ui-ux-intelligence status`, `kvdf ui-ux-intelligence source-status`, `kvdf ui-ux-intelligence catalog`, `kvdf ui-ux-intelligence search`, `kvdf ui-ux-intelligence recommend`, `kvdf ui-ux-intelligence design-system`, `kvdf ui-ux-intelligence checklist`, `kvdf ui-ux-intelligence docs`, `kvdf ui-ux-intelligence audit`, `kvdf ui-ux-intelligence scorecard`, `kvdf ui-ux-intelligence gate`, `kvdf ui-ux-intelligence readiness`, `kvdf ui-ux-intelligence handoff-pack`, `kvdf ui-ux-intelligence tokens`, `kvdf ui-ux-intelligence components`, `kvdf ui-ux-intelligence screens`, `kvdf plugins install ui_ux_intelligence`, `kvdf plugins uninstall ui_ux_intelligence` |
