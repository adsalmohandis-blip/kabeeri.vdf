...
| Planner Layer | Shared native planning capability that reads repo and runtime context, then recommends the next governed Evolution, emits a Task Punch, and prepares a Codex-ready prompt for owner, vibe/app, or plugin tracks. KVDF Core stays direct-to-main by default, vibe/app planning stays local-first by default, plugin planning protects plugin mount/runtime state, and the MVP does not write runtime state under `.kabeeri/`. | `knowledge/governance/KVDF_PLANNER_LAYER.md`, `docs/workflows/EVOLUTION_PLANNER_WORKFLOW.md`, `packs/planner/`, `schemas/planner/`, `kvdf planner next`, `kvdf planner prompt`, `kvdf planner evolution`, `kvdf planner task-punch` |
