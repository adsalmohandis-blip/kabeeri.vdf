...
Kabeeri now includes a Structured Delivery Runtime. The runtime turns
requirements, phases, deliverables, risks, change requests, and phase gates into
workspace records under `.kabeeri/structured.json`.

Main commands:

```bash
kvdf structured health
kvdf structured requirement add
kvdf structured requirement approve
kvdf structured phase plan
kvdf structured task <requirement-id>
kvdf structured gate check <phase-id>
kvdf structured phase complete <phase-id>
kvdf validate structured
```

See [STRUCTURED_RUNTIME.md](STRUCTURED_RUNTIME.md) for the executable model.
