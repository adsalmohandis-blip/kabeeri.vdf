...
- حزم `plugin` تعرض الآن `bundle contract` داخل `kvdf plugins status` لعرض المجلدات المطلوبة، ومجلدات النوع، وأول خطوة تالية بشكل واضح قبل أي تنفيذ
...
قبل أي تنفيذ جديد، استخدم `kvdf contract` أو `kvdf resume` حتى ترى الخطوة التالية بشكل واضح وتبقى ضمن مسار governance بدل التخمين.
