...
Configure environment, database, Doctrine, and local setup rules.
...
- Database: SQLite, PostgreSQL, MySQL/MariaDB, or not decided
- Is this local-only for now or preparing for deployment?
- Do you need mailer, storage, or messenger now?
- Technical note: If unsure, ask ChatGPT to compare Symfony database choices for your product.
...
5. Prepare Doctrine/DBAL foundation only if needed now.
6. Keep SQLite acceptable for Lite/local projects unless another database is selected.
7. Do not create product-specific entities yet.
